...
def monitor_system():
    ...
    try:
        while True:
            ...
            print(f"   Used:  {format_memory(system_mem.used / 1024 / 1024)} ({system_mem.percent:.1f}%)")
            ...
            if backend_processes:
                ...
                for proc in backend_processes:
                    if 'main:app' in proc['cmdline'] or 'uvicorn' in proc['cmdline']:
                        print(f"   PID {proc['pid']}: {format_memory(proc['memory_mb'])} - {proc['cmdline']}")
            ...
            try:
                ...
                if gpus:
                    ...
                    for gpu in gpus:
                        ...
                        print(f"   {gpu.name}: {used_mb} MB / {total_mb} MB ({gpu.memoryUtil*100:.1f}%)")
                else:
            ...
            print("\n🎯 Performance Targets:")
            print(f"   Server Memory Target: <2GB (Current: {format_memory(total_backend_memory)})")
            print(f"   Browser Memory Target: <500MB (Check browser dev tools)")
