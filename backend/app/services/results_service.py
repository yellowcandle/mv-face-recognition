...
class ResultsService:
    ...
    async def get_contestant_appearances(self, job_id: str) -> Optional[List[ContestantStats]]:
    ...
    async def search_frames(
        ...
        if min_confidence > 0:
            frames = [
                frame for frame in frames
                if any(face.recognition_confidence >= min_confidence for face in frame.faces)
            ]
