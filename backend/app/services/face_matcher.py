...
class FaceMatcherAsync:
    ...
    async def match_face_async(self, embedding: np.ndarray) -> Optional[Tuple[str, float]]:
        """
    ...
    async def match_faces_batch_async(self, embeddings: List[np.ndarray]) -> List[Optional[Tuple[str, float]]]:
        """
    ...
    async def get_top_matches_async(self, embedding: np.ndarray, top_k: int = 5) -> List[Tuple[str, float]]:
        """
