    ...
    async def process_video_realtime_async(
        ...
        try:
            ...
            async for frame_num, frame in self._extract_frames_async(
                ...
                max_concurrent_faces = min(
                    len(faces), 4
                )  # Limit to 4 concurrent face processes
                face_results = []
                ...
                current_frame_recognized = (
                    sum(1 for result in face_results if result["matched"])
                    if face_results
                    else 0
                )
                ...
                preview_frame = await self._create_preview_frame_async(
