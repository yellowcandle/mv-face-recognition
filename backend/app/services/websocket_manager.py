...
class WebSocketManager:
    ...
    async def send_processing_update(
        self, websocket: WebSocket, frame_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            await self.send_to_client(
                websocket, {"type": "processing_update", "data": optimized_data}
            )

        except Exception as e:
            logger.error(f"Error sending processing update: {e}")

    async def send_frame_image(
        self, websocket: WebSocket, frame_bytes: bytes, frame_info: Dict
    ):
        """Send frame image data efficiently."""
        try:
            # Send frame info first
            await self.send_to_client(
                websocket, {"type": "frame_info", "data": frame_info}
            )
