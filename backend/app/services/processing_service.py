...
class ProcessingService:
    ...
    async def start_processing_job(
        self, video_id: str, config: ProcessingConfig, background_tasks: BackgroundTasks
    ) -> ProcessingJob:
    ...
    async def get_processing_stats(self) -> Dict[str, Any]:
        ...
        completed = sum(
            1 for job in self.jobs.values() if job.status == ProcessingStatus.COMPLETED
        )
        failed = sum(
            1 for job in self.jobs.values() if job.status == ProcessingStatus.FAILED
        )
        processing = sum(
            1 for job in self.jobs.values() if job.status == ProcessingStatus.PROCESSING
        )
