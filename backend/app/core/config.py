...
class Settings(BaseSettings):
    ...
    videos_dir: str = (
        "/data/videos" if os.getenv("FLY_APP_NAME") else "../source/videos"
    )
    contestants_dir: str = (
        "/data/embeddings/contestants"
        if os.getenv("FLY_APP_NAME")
        else "../source/photo/contestants"
    )
    output_dir: str = "/data/output" if os.getenv("FLY_APP_NAME") else "../output"
    config_file: str = (
        "/app/config.json" if os.getenv("FLY_APP_NAME") else "../config.json"
    )
    ...
    # Database settings
    chroma_db_path: str = (
        "/data/chroma_db" if os.getenv("FLY_APP_NAME") else "../data/chroma_db"
    )
