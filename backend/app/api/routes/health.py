...
async def health_check():
    ...
    try:
        ...
        volumes = ["/data/videos", "/data/embeddings", "/data/metadata", "/data/chroma_db"]
        for volume in volumes:
            ...
            if Path(volume).exists():
                ...
                try:
                    ...
                    health_status["system"]["disk_usage"][volume] = {"error": "Cannot access"}
        ...
        for emb_path in embeddings_paths:
            ...
            if emb_dir.exists():
                ...
                health_status["critical_files"]["embeddings_count"] = len(contestant_dirs)
                break
        ...
        health_status["services"]["file_system"] = (
            health_status["critical_files"]["contestant_info_csv"] and
            health_status["critical_files"]["config_json"] and
            health_status["critical_files"]["embeddings_count"] > 90  # Expect 95+ contestants
        )
    ...
    if not all(critical_services):
        ...
        if health_status["critical_files"]["embeddings_count"] <= 90:
            health_status["issues"].append(f"WARNING: Only {health_status['critical_files']['embeddings_count']} contestant embeddings found (expected 95+)")
    ...
    try:
        ...
        if os.getenv("FLY_APP_NAME"):
            ...
            if not volumes_ready:
                raise HTTPException(
                    status_code=503, 
                    detail="Storage volumes not ready"
                )
        ...
        if not contestant_info_exists:
            raise HTTPException(
                status_code=503,
                detail="CRITICAL: contestant_info.csv not found"
            )
