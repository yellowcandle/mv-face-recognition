...
async def list_results(
    ...
    try:
        results = await results_service.get_results(
            video_id=video_id, limit=limit, offset=offset
        )
        return results
...
async def get_result(
    job_id: str, results_service: ResultsService = Depends(get_results_service)
):
...
async def get_frame_results(
    ...
    try:
        frames = await results_service.get_frame_results(
            job_id=job_id, start_frame=start_frame, end_frame=end_frame
        )
        if frames is None:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get frame results: {str(e)}"
        )
...
async def get_contestant_appearances(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get contestant stats: {str(e)}"
        )
...
async def search_frames(
    job_id: str,
    contestant_name: Optional[str] = Query(
        None, description="Search by contestant name"
    ),
    min_confidence: float = Query(
        0.0, ge=0.0, le=1.0, description="Minimum confidence score"
    ),
    start_time: Optional[float] = Query(
        None, ge=0.0, description="Start time in seconds"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to search frames: {str(e)}"
        )
