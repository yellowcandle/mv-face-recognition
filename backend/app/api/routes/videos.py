...
async def get_dense_video_metadata(video_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error reading dense metadata: {str(e)}")
...
async def generate_dense_metadata(video_id: str):
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to generate dense metadata: {result['error']}")
            
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error generating dense metadata: {str(e)}")
    ...
    try:
        with open(csv_file, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                has_photos = photo_dir.exists() and len(list(photo_dir.glob("*.jpg"))) > 0
                
                # Check if contestant has embedding
                embedding_file = Path(f"source/photo/contestants/{row['暱稱']}_embedding.npy")
                ...
                contestants.append({
                    ...
                    "photo_url": f"/api/contestants/{row['編號']}/photo" if has_photos else None
                })
    
    except (IOError, UnicodeDecodeError) as e:
        raise HTTPException(status_code=500, detail=f"Error reading contestant info: {str(e)}")
