...
class PoseRecognition:
    ...
    def get_pose_similarity(self, angles1: Tuple[float, float, float], 
        ...
        weighted_similarity = float(np.sum(normalized_diffs * weights))  # Convert to float
    ...
    def get_pose_bbox(self, image: np.ndarray) -> Optional[np.ndarray]:
        """
        Get bounding box for the detected pose.
        
        Args:
            image: Input image in BGR format
            
        Returns:
            numpy.ndarray: Array of [x1, y1, x2, y2] coordinates or None if no face detected
        """
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        faces = self.detector(gray)
        ...
        padding = 30  # Pixels to add around the landmarks
        x1 = max(0, int(np.min(points[:, 0]) - padding))
        y1 = max(0, int(np.min(points[:, 1]) - padding))
        x2 = min(image.shape[1], int(np.max(points[:, 0]) + padding))
        y2 = min(image.shape[0], int(np.max(points[:, 1]) + 2 * padding))  # Extra padding below for body
