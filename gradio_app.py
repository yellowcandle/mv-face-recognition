    ...
    def _calculate_contestant_stats(self):
        ...
        for video_name, metadata in self.video_metadata.items():
            ...
            for contestant, data in contestant_timeline.items():
                ...
                stats['avg_confidence'] = max(stats['avg_confidence'], data['avg_confidence'])
                stats['best_confidence'] = max(stats['best_confidence'], data['max_confidence'])
...
"""
        ...
        if contestant_timeline:
            ...
            for contestant, data in sorted_contestants[:10]:  # Top 10
                ...
                timeline_text += f"- First seen: {data['first_appearance_time']:.1f}s\\n"
                timeline_text += f"- Last seen: {data['last_appearance_time']:.1f}s\\n\\n"
        else:
            timeline_text = "No contestants detected in this video."
        
        return info_text, timeline_text
    
    def get_contestant_clips(self, contestant_filter: str = "All") -> List[Tuple[str, str]]:
...
"""
        ...
        for video_name, metadata in self.video_metadata.items():
            ...
            video_stats += f"- **Faces Recognized:** {summary['total_faces_recognized']}\\n"
            video_stats += f"- **Recognition Rate:** {summary['recognition_rate']:.1%}\\n"
            video_stats += f"- **Unique Contestants:** {summary['unique_contestants']}\\n\\n"
    ...
    def process_video_with_progress(self, video_name: str, similarity_threshold: float) -> Generator[Dict, None, None]:
        ...
        try:
            ...
            yield {
                ...
                'video_name': video_name,
                'stats': {
                    'faces_detected': recognition_results.get('total_faces_detected', 0),
                    'faces_recognized': recognition_results.get('total_faces_recognized', 0),
                    'frames_skipped': recognition_results.get('total_frames_skipped', 0)
                }
            }
            ...
            yield {
                ...
                'stats': {
                    ...
                    'faces_recognized': recognition_results.get('total_faces_recognized', 0),
                    'frames_skipped': recognition_results.get('total_frames_skipped', 0),
                    'unique_contestants': len(recognition_results.get('contestant_appearances', {}))
                }
            }
            
            clips_info = self.video_processor.extract_highlight_clips(video_name, recognition_results)
            ...
            metadata = self.video_processor.generate_video_metadata(video_name, recognition_results)
            metadata_file = self.video_processor.metadata_dir / f"{Path(video_name).stem}_metadata.json"
    ...
    def format_processing_stats(self, stats: Dict) -> str:
        ...
        if 'faces_detected' in stats and 'faces_recognized' in stats and stats['faces_detected'] > 0:
        ...
        if self.video_processor:
            try:
                ...
                if detector_info:
                    ...
                    if providers:
                        info_text += f"**Execution Providers:**\\n"
                    ...
                    if system_info:
                        ...
                        if detector_info.get('apple_silicon'):
                            info_text += f"- Apple Silicon: ✅ Detected\\n"
                        if detector_info.get('cuda_available'):
                            info_text += f"- CUDA: ✅ Available\\n"
    ...
    def create_interface(self):
        ...
        with gr.Blocks(css=css, title="MV Face Recognition", theme=gr.themes.Soft()) as demo:
            gr.Markdown("""
            ...
            **Intelligent contestant recognition in music videos using AI face detection.**
            ...
            with gr.Tabs():
                    ...
                    with gr.Row():
                        with gr.Column(scale=2):
                            video_selector = gr.Dropdown(
                                ...
                                label="Select Video",
                                value=self.get_available_videos()[0] if self.get_available_videos() else None,
                                interactive=True
                            )
                ...
                with gr.TabItem("🎬 Process Videos", id="process_videos"):
                    ...
                    def process_video_gradio(video_name, threshold, progress=gr.Progress()):
                        if not video_name:
                            return (
                                "No video selected",
                        ...
                        for update in self.process_video_with_progress(video_name, threshold):
                    ...
                    refresh_btn.click(
                        fn=refresh_source_videos,
                        outputs=[source_video_selector]
                    )
                ...
                with gr.TabItem("📊 Analytics", id="analytics"):
                    ...
                    demo.load(
                        ...
                        outputs=[overall_stats, video_stats]
                    )
