...
def process_frame(frame, known_embeddings, similarity_threshold=0.5):
    ...
    try:
        ...
        if min_dimension < 720:
            ...
            logger.debug(f"Scaled frame from {w}x{h} to {new_w}x{new_h} for better face detection")
        elif min_dimension > 1920:
            # Scale down very large videos for performance
            scale_factor = 1920 / min_dimension
            new_h, new_w = int(h * scale_factor), int(w * scale_factor)
            frame = cv2.resize(frame, (new_w, new_h), interpolation=cv2.INTER_AREA)
            logger.debug(f"Scaled frame from {w}x{h} to {new_w}x{new_h} for performance")
        else:
            scale_factor = 1.0
        ...
        for face in faces:
            try:
                ...
                if face_embedding is None:
                    ...
                    if x2 > x1 and y2 > y1:
                        face_crop = original_frame[y1:y2, x1:x2]  # Use original frame for embedding
                        if face_crop.size > 0:
                    ...
                    for name, known_embedding in known_embeddings.items():
                        try:
                            ...
                            if face_embedding.shape[0] != known_embedding.shape[0]:
                                logger.debug(f"Embedding dimension mismatch for {name}: {face_embedding.shape} vs {known_embedding.shape}")
                                continue
...
def calculate_face_similarity(bbox1, bbox2):
    ...
    similarity_score = (0.4 * normalized_distance + 0.3 * (1 - size_ratio) + 0.3 * (1 - iou))
    ...
    for i, label_info in enumerate(labels_to_draw):
        ...
        while collision_found and attempts < 10:
            ...
            for prev_label in positioned_labels:
                ...
                if (best_x < prev_x + prev_w and best_x + text_width > prev_x and
                    best_y < prev_y + prev_h and best_y + text_height > prev_y):
                    collision_found = True
                    break
...
def update_persistent_labels(matches, label_cache, current_frame, persistence_duration):
    ...
    expired_keys = [key for key, data in label_cache.items() 
                   if data['expire_frame'] < current_frame]
    ...
    for face, name in matches:
        ...
        for existing_key, existing_data in label_cache.items():
            ...
            if similarity < 0.8 and similarity < best_similarity:  # More lenient threshold for smoother tracking
        ...
        label_cache[face_key] = {
            ...
            'confidence': getattr(face, 'det_score', 1.0) if hasattr(face, 'det_score') else 1.0
        }
    
    return label_cache

def draw_boxes_and_labels(frame, matches, timestamp="", persistent_labels=None, current_frame=0):
    ...
    try:
            ...
            try:
                        ...
                        try:
                            ...
                            for size in [16, 14, 12, 10]:
                                ...
                                for char in test_chars:
                                    try:
                                        bbox = test_draw.textbbox((0, 0), char, font=test_font)
                                        if bbox[2] - bbox[0] <= 0:  # No width means can't render
                ...
                if pil_font is None:
                    ...
                    try:
                        ...
                        for font_file in system_fonts[:20]:  # Test first 20 system fonts
                            try:
        ...
        for face, name in matches:
            ...
            confidence = getattr(face, 'det_score', 0.8) if hasattr(face, 'det_score') else 0.8
        ...
        if persistent_labels:
            for face_key, label_data in persistent_labels.items():
                ...
                for face, _ in matches:
                    ...
                    if calculate_face_similarity(label_data['bbox'], current_bbox) < 0.3:
                        overlaps = True
                        break
                ...
                if not overlaps:
                    ...
                    tracking_duration = current_frame - (label_data.get('last_seen', current_frame) - 10)
                    tracking_duration = max(1, min(tracking_duration, 90))  # Cap at persistence duration
        ...
        # Calculate optimal label positions to avoid collisions
        positioned_labels = calculate_label_positions(all_labels_to_draw, frame_pil.width, frame_pil.height)
        ...
        for positioned_label in positioned_labels:
            ...
            adaptive_opacity = calculate_adaptive_opacity(tracking_duration, is_current, confidence)
                        ...
                        try:
                            ...
                            if bbox[2] - bbox[0] > 0:  # Character has width
                                ...
                                if ord(char) > 127:  # Non-ASCII
                                    sanitized_chars.append('?')  # Use ? for unrenderable chars
                                else:
                                    sanitized_chars.append(char)
                        except Exception:
            ...
            overlay_draw.rectangle(background_coords, fill=color_rgb + (background_alpha,))
        ...
        try:
            for face, name in matches:
                ...
                cv2.putText(frame, ascii_name, (x1, y1-10), 
...
class FaceRecognitionApp:
            ...
            for embedding_file in embeddings_dir.glob("*_embedding.npy"):
                try:
                    ...
                    if np.linalg.norm(embedding) > 0:
                        ...
                        logger.warning(f"Invalid embedding for {contestant_name} - zero norm")
                        
                except Exception as e:
                    logger.warning(f"Failed to load embedding for {embedding_file}: {e}")
                    continue
                        ...
                        if frame_count > 0:
                            ...
                            if file_stem in video_titles:
                                ...
                                video_number_mapping = {
                                    'v1': list(video_titles.keys())[0] if len(video_titles) > 0 else None,
                                    'v2': list(video_titles.keys())[1] if len(video_titles) > 1 else None,
                                    'v3': list(video_titles.keys())[2] if len(video_titles) > 2 else None,
                                    'v4': list(video_titles.keys())[3] if len(video_titles) > 3 else None,
                                }
                            ...
                            if matching_title:
                                ...
                                if matching_title not in title_to_path_mapping or file_stem == matching_title:
                                    if matching_title not in video_files:
                                        video_files.append(matching_title)
                                    title_to_path_mapping[matching_title] = str(video_file)
                            else:
    ...
    def _create_interactive_timeline(self, timeline_df, video_path):
        ...
        try:
            ...
            if not all(col in timeline_df.columns for col in required_columns):
                raise ValueError(f"DataFrame missing required columns. Expected: {required_columns}, Got: {list(timeline_df.columns)}")
            ...
            try:
                ...
                for name in names:
                    ...
                    if time_seconds:  # Only add trace if we have valid time data
                        fig.add_trace(go.Scatter(
                            ...
                            name=name,
                            text=[f"Frame {frame}: {name} at {time}" 
                                  for frame, time in zip(name_data['Frame'], name_data['Time'])],
                ...
                plotly_html = fig.to_html(include_plotlyjs='cdn', div_id="timeline_chart")
                return table_html + plotly_html
                
            except Exception as plotly_error:
                logger.debug(f"Plotly visualization failed, using table only: {plotly_error}")
                return table_html
            ...
            try:
                ...
                for _, row in timeline_df.iterrows():
                    timeline_text += f"• {row['Time']} - {row['Name']} (Frame {row['Frame']})\n"
    ...
    def generate_umap_visualization(self, detected_faces_data):
        ...
        if not self.visualization_service:
            ...
            ax.text(0.5, 0.5, "UMAP visualization not available\n(visualization service not initialized)", 
        ...
        try:
            ...
            for i, (face, name) in enumerate(detected_faces_data):
                ...
                if embedding is not None:
                    ...
                    if len(embedding) > 0 and not np.isnan(embedding).any():
                        ...
                        logger.debug(f"Added embedding for {name} (shape: {embedding.shape})")
                    else:
                        logger.warning(f"Invalid embedding for {name}: contains NaN or empty")
                else:
                    logger.warning(f"No embedding found for face {i} ({name})")
            
            logger.info(f"Extracted {len(detected_embeddings)} valid embeddings from {len(detected_faces_data)} faces")
            
            if len(detected_embeddings) == 0:
                # No valid embeddings found
                import matplotlib.pyplot as plt
                fig, ax = plt.subplots(figsize=(10, 8))
                ax.text(0.5, 0.5, f"No valid face embeddings found\nfrom {len(detected_faces_data)} detected faces\n\nThis could happen if:\n• Face detection didn't extract embeddings\n• Embeddings are corrupted\n• Face crops are too small", 
            ...
            if valid_gallery_count == 0:
                import matplotlib.pyplot as plt
                fig, ax = plt.subplots(figsize=(10, 8))
                ax.text(0.5, 0.5, "No gallery embeddings available\nPlease ensure contestant embeddings are loaded", 
            ...
            import matplotlib.pyplot as plt
            fig, ax = plt.subplots(figsize=(10, 8))
            error_text = f"Error generating UMAP:\n{str(e)}\n\nCheck console for details"
            ax.text(0.5, 0.5, error_text, ha='center', va='center', fontsize=11, color='red')
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
    ...
    def process_uploaded_video(self, video_path, progress=gr.Progress()):
        ...
        try:
            ...
            if not cap.isOpened():
                return None, f"Error: Cannot open video file {video_path}. The file may be corrupted or in an unsupported format."
            ...
            if total_frames <= 0 or fps <= 0:
                cap.release()
                return None, f"Error: Invalid video file {video_path}. Cannot read video properties."
            ...
            while cap.isOpened():
                ...
                if should_process:
                    ...
                    label_persistence_cache = update_persistent_labels(
                        matches, label_persistence_cache, frame_count, label_persistence_duration
                    )
                    
                    # Record matches
                    faces_detected = len(matches)
                    faces_recognized = sum(1 for _, name in matches if name != "Unknown")
                    ...
                    if frame_count % 500 == 0:
                        logger.info(f"Frame {frame_count}: Detected {faces_detected} faces, recognized {faces_recognized}")
                ...
                if matches or label_persistence_cache:
                    frame = draw_boxes_and_labels(frame, matches, time_str, label_persistence_cache, frame_count)
                ...
                timestamp = frame_count / fps
                time_str = f"{int(timestamp//60):02d}:{int(timestamp%60):02d}"
                cv2.putText(frame, time_str, (10, frame.shape[0] - 20), 
            ...
            if results:
                ...
                summary += f"• Faces detected: {self.processing_stats['faces_detected']}\n"
                summary += f"• Faces recognized: {self.processing_stats['faces_recognized']}\n"
                ...
                summary += f"• Faces detected: {self.processing_stats['faces_detected']}\n"
                summary += f"• Faces recognized: {self.processing_stats['faces_recognized']}\n"
                summary += f"• Recognition events: 0\n\n"
            ...
            if results:
                ...
                timeline_df.columns = ['Frame', 'Time', 'Name']  # Rename to match expected format
                timeline_html = self._create_interactive_timeline(timeline_df, output_path)
            ...
            return None, f"Error processing video: {e}", "Error occurred during processing"
    ...
    def get_system_status(self):
        ...
        try:
            status = {
                ...
                'ChromaDB': 'Connected' if self.config.recognition.enable_chromadb else 'Disabled',
                'GPU Acceleration': 'Enabled' if self.config.recognition.use_gpu else 'Disabled',
                ...
                'Last Updated': self.processing_stats['last_updated'].strftime("%H:%M:%S")
            }
    ...
    def get_performance_chart(self):
        ...
        try:
            ...
            data = {
                ...
                'Count': [stats['total_processed'], stats['faces_detected'], stats['faces_recognized']]
            }
            
            fig = px.bar(data, x='Metric', y='Count', 
    ...
    def update_settings(self, similarity_threshold, detection_threshold, frame_skip, 
                       max_faces, use_gpu, enable_chromadb):
    ...
    def add_contestant_embedding(self, contestant_name, image):
        ...
        try:
            ...
            # Save embedding
            embedding_path = self.config.contestants_dir / f"{contestant_name}_embedding.npy"
        ...
        with gr.Tabs():
            ...
            with gr.Tab("🎬 Video Processing"):
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=1):
                        with gr.Group():
                            ...
                            gr.Markdown("*Available videos will show with their full titles from the video collection*")
                ...
                def process_selected_video(dropdown_title):
                    ...
                    if not dropdown_title:
                        return None, "Please select a video from the dropdown", "No video selected"
                ...
                def refresh_video_dropdown():
                    ...
                    return gr.Dropdown(choices=get_app().get_video_titles_for_dropdown())
            ...
            with gr.Tab("⚙️ Settings"):
                ...
                settings_button.click(
                    ...
                    inputs=[sim_threshold, det_threshold, frame_skip, max_faces, 
                           use_gpu, enable_chromadb],
            ...
            with gr.Tab("👥 Contestants"):
                ...
                demo.load(
                    lambda: get_app().get_contestant_list(),
                    outputs=[contestant_list]
                )
            
            # UMAP Visualization Tab
            with gr.Tab("🗺️ UMAP Visualization"):
                gr.Markdown("## 🗺️ Face Embedding Visualization (UMAP)")
                gr.Markdown("*Visualize relationships between detected faces and known contestants using UMAP dimensionality reduction*")
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=1):
                        with gr.Group():
                            ...
                            umap_status = gr.HTML("Select a video and click Generate UMAP")
                ...
                def generate_umap_for_video(video_title):
                    ...
                    try:
                        ...
                        similarity_threshold = app.config.recognition.similarity_threshold
                        ...
                        logger.info(f"Processing frames with skip={frame_skip} (every {frame_skip} frames) from {total_frames} total frames for UMAP analysis")
                        ...
                        while cap.isOpened():
                            ...
                            if should_process:
                                ...
                                matches = process_frame(frame, app.known_embeddings, similarity_threshold)
                                if matches:
                                ...
                                if processed_count % 100 == 0:
                                    logger.info(f"Processed {processed_count} frames (frame {frame_count}/{total_frames}), found {faces_found_count} faces so far")
                        
                        logger.info(f"UMAP processing complete: {faces_found_count} total faces from {processed_count} processed frames (frame skip: {frame_skip})")
                        
                        cap.release()
                        
                        if not all_matches:
                            return None, f"❌ No faces detected in {processed_count} processed frames (every {frame_skip} frames)"
                        
                        # Generate UMAP visualization
                        logger.info(f"Generating UMAP for {len(all_matches)} face detections from video")
                        umap_fig = app.generate_umap_visualization(all_matches)
                        
                        unique_faces = len(set([name for _, name in all_matches if name != "Unknown"]))
                        
                        return umap_fig, f"✅ Generated UMAP from {processed_count} frames (every {frame_skip}): {len(all_matches)} face detections ({unique_faces} unique people)"
            ...
            with gr.Tab("📊 Analytics"):
                ...
                refresh_button.click(
                    get_app().get_performance_chart,
                    outputs=[analytics_chart]
                )
                
                refresh_button.click(
                    get_app().get_system_status,
