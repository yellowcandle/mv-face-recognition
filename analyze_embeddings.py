...
def main():
    ...
    if len(recognized) == 2 and all(name in names for name in recognized):
        ...
        avg_dist_1 = np.mean(
            [distances[idx1, i] for i in range(len(names)) if i != idx1]
        )
        avg_dist_2 = np.mean(
            [distances[idx2, i] for i in range(len(names)) if i != idx2]
        )

        print("\nRecognized pair analysis:")
        print(
            f"Distance between {recognized[0]} and {recognized[1]}: {between_distance:.4f}"
        )
    ...
    print("Recognition in video shows distances ~7.6-7.9, confidences ~0.2-0.24")
    print("Current threshold: 0.05 (very permissive)")
