...
def parse_args():
    parser = argparse.ArgumentParser(description="Synthetic benchmark for ChromaDB vs standard matching")
    
    parser.add_argument("--embeddings", type=int, default=100, 
...
def chromadb_matching(query_embeddings, reference_embeddings, threshold=0.6, verbose=False, iteration=0):
...
def run_benchmark(args):
    ...
    print(f"Running synthetic benchmark with:")
    ...
    if HAS_CHROMADB:
        ...
        print(f"ChromaDB matching:")
        print(f"  Average time: {avg_chroma_time:.4f}s")
        print(f"  Average matches: {avg_chroma_matches:.1f}")
        print(f"  Speedup: {speedup:.2f}x")
        
        # Match consistency
        match_similarity = abs(avg_std_matches - avg_chroma_matches) / max(avg_std_matches, avg_chroma_matches)
    ...
    if args.embeddings <= 1000:
        ...
        for count in embedding_counts:
            ...
            query_embeddings = create_synthetic_embeddings(10, args.dimensions)  # Use fewer queries for scaling test
            
            # Standard matching
            _, std_time = standard_matching(query_embeddings, ref_embeddings, threshold=args.threshold)
            std_scaling_times.append(std_time)
            
            # ChromaDB matching
            if HAS_CHROMADB:
                _, chroma_time = chromadb_matching(query_embeddings, ref_embeddings, threshold=args.threshold, iteration=count)
                chroma_scaling_times.append(chroma_time)
            
            print(f"Standard: {std_time:.4f}s, ChromaDB: {chroma_time:.4f}s" if HAS_CHROMADB else f"Standard: {std_time:.4f}s")
        
        # Print scaling results
        print("\nScaling Results:")
        for i, count in enumerate(embedding_counts[:len(std_scaling_times)]):
            if HAS_CHROMADB and i < len(chroma_scaling_times):
                speedup = std_scaling_times[i] / chroma_scaling_times[i] if chroma_scaling_times[i] > 0 else 0
                print(f"  {count} embeddings: Standard={std_scaling_times[i]:.4f}s, ChromaDB={chroma_scaling_times[i]:.4f}s, Speedup={speedup:.2f}x")
            else:
