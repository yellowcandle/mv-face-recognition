    ...
    try:
        ...
        try:
            ...
            if torch.cuda.is_available():
                console.print(
                    f"[green]⚡ {container_id}[/green] GPU: {torch.cuda.get_device_name()}"
                )
            else:
        ...
        try:
            ...
            config["paths"]["contestants_dir"] = str(
                VOL_MOUNT_PATH / "source/photo/contestants"
            )
        ...
        try:
            ...
            if hasattr(processor, "face_matcher") and hasattr(
                ...
                console.print(
                    f"[cyan]📊 {container_id}[/cyan] DB Stats: {db_stats['total_embeddings']} embeddings loaded"
                )
                console.print(
                    f"[cyan]🎯 {container_id}[/cyan] Similarity threshold: {db_stats['similarity_threshold']}"
                )
            else:
                console.print(
                    f"[yellow]⚠️ {container_id}[/yellow] Could not access database stats"
                )
        ...
        try:
            HardwareAccelerator()
            console.print(
                f"[green]🔧 {container_id}[/green] Hardware acceleration active"
            )
        except Exception:
        ...
        console.print(
            f"[bold green]🎬 {container_id}[/bold green] Starting video processing..."
        )
...
def launch_parallel_processing(
    ...
    console.print(
        f"[bold blue]🚀 Launching {len(video_list)} containers with starmap...[/bold blue]"
    )

    # Create parameter tuples for starmap
    video_params = [
        (video_name, similarity_threshold, force_reprocess) for video_name in video_list
    ]
    ...
    console.print(
        f"[bold green]✅ All {len(video_list)} containers completed in {elapsed:.1f}s![/bold green]"
    )
...
def main(
    ...
    if not 0.0 <= similarity_threshold <= 1.0:
        console.print(
            "[red]Error: Similarity threshold must be between 0.0 and 1.0[/red]"
        )
        sys.exit(1)
    ...
    if single_video:
        ...
        console.print(
            f"[bold blue]🎬 Processing single video: {single_video}[/bold blue]"
        )
        ...
        with Progress(
            ...
            process_single_video_rich.remote(
        ...
        with Progress(
            ...
            progress.add_task("Fetching videos...", total=None)
...
def print_rich_summary(
    console, results: Dict[str, Any], total_time: float, containers_used: int
):
    ...
    if successful:
        ...
        total_faces_detected = sum(
            results[v].get("stats", {}).get("total_faces_detected", 0)
            for v in successful
        )
        total_faces_recognized = sum(
            results[v].get("stats", {}).get("total_faces_recognized", 0)
            for v in successful
        )
        total_frames_processed = sum(
            results[v].get("stats", {}).get("total_frames_processed", 0)
            for v in successful
        )
        total_frames_skipped = sum(
            results[v].get("stats", {}).get("total_frames_skipped", 0)
            for v in successful
        )
        ...
        skip_rate = (
            total_frames_skipped / max(total_frames_processed + total_frames_skipped, 1)
        ) * 100
    else:
        total_proc_time = speedup = recognition_rate = skip_rate = 0
        total_faces_detected = total_faces_recognized = total_frames_processed = (
