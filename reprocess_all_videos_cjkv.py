...
class EnhancedVideoReprocessor:
    ...
    def initialize_pipeline(self):
        ...
        try:
            ...
            self.pipeline = VideoProcessingPipeline(self.config_path, enable_upload=False)
            
            logger.info("📊 Loading contestant database...")
            self.pipeline.initialize_database()
            
            logger.info(f"✅ Pipeline initialized with {len(self.pipeline.contestant_db.face_encodings)} contestants")
            return True
    ...
    def process_all_videos(self):
        ...
        logger.info("🚀 Starting enhanced video reprocessing with CJKV fonts and audio merging")
        start_time = time.time()
        ...
        # Final summary
        logger.info("=" * 60)
        logger.info(f"🎉 Video reprocessing completed!")
        logger.info(f"✅ Successful: {success_count}/{total_videos} videos")  
        logger.info(f"⏱️  Total time: {total_time/60:.1f} minutes")
        logger.info(f"📊 Average time per video: {total_time/total_videos:.1f} seconds")
