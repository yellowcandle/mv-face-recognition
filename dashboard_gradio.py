    ...
    def process_video_real_time(self, video_title, progress=gr.Progress()):
        ...
        try:
            ...
            if output_video_path and results:
                ...
                summary = f"✅ Processing complete! Found {len(results)} recognition events with {len({r.get('Name', 'Unknown') for r in results if r.get('Name') != 'Unknown'})} unique contestants."
        ...
        for tile in self.face_tiles_data:
            ...
            " onclick="selectFace('{tile['id']}')">
                ...
                ">
                    ...
                    👤
                    {'<div style="position: absolute; top: 2px; right: 2px; width: 8px; height: 8px; background: #3b82f6; border-radius: 50%; animation: pulse 1.5s infinite;"></div>' if is_selected else ''}
                </div>
...
def create_dashboard_interface():
    ...
    with gr.Blocks(
        ...
        dashboard.create_header_component()
