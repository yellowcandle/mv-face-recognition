...
def find_contestant_photos(contestants_dir, contestant_id):
    ...
    for dir_path in glob.glob(
        os.path.join(contestants_dir, "*")
    ):  # Iterate through numbered directories like "1", "2", ...
...
def main():
    ...
    print(
        f"[DEBUG] generate_embeddings: FaceRecognizer configured embedding_size: {face_recognizer.embedding_size}"
    )
            ...
            try:
                ...
                if face_img is not None:
                    ...
                    if current_embedding is None or current_embedding.size == 0:
                        print(
                            f"Failed to compute embedding via face_recognizer._get_embedding for {nickname} from {photo_path}"
                        )
                        current_embedding = None  # Ensure it's None if failed
                else:
                    print(
                        f"Could not extract face_img for {nickname} from {photo_path} using bbox {bbox_to_extract}"
                    )
