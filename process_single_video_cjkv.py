...
def process_video_enhanced(video_path, output_name, config_path):
    ...
    # Create thumbnail
    thumbnail_path = Path(config["output"]["thumbnails_dir"]) / f"{output_name}_thumb.jpg"
    ...
    # Filter recognitions
    filtered_recognitions = face_recognizer.filter_recognitions(all_recognitions, min_confidence=0.5)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python process_single_video_cjkv.py <video_path> <output_name> <config_path>")
        sys.exit(1)
