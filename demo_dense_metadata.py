...
def compare_metadata_density():
    ...
    if dense_files:
        ...
        print(
            f"Interpolation enabled: {processing_info.get('interpolation_enabled', 'N/A')}"
        )
        print(
            f"Processing time: {processing_info.get('processing_time_seconds', 'N/A'):.2f}s"
        )
        print()
        ...
        for contestant_name, timeline in contestant_timeline.items():
            ...
            interpolated_count = sum(
                1 for entry in detailed_timeline if entry.get("interpolated", False)
            )
            ...
            print(
                f"  Interpolation ratio: {interpolated_count/len(detailed_timeline)*100:.1f}%"
            )
            ...
            if detailed_timeline:
                ...
                print(
                    f"  Time coverage: {start_time:.1f}s - {end_time:.1f}s ({end_time-start_time:.1f}s duration)"
                )
            print()
...
def show_processing_improvements():
    ...
    print(
        f"- Frame skip: {config['video_processing']['frame_skip']} (processes every {config['video_processing']['frame_skip']}th frame)"
    )
    print("- No interpolation between detections")
    print("- Gaps in timeline data")
    print()

    print("NEW DENSE PROCESSING:")
    processor = RealtimeVideoProcessor()
    print(
        f"- Frame interval: {processor.dense_frame_interval} (processes every {processor.dense_frame_interval}th frame)"
    )
    print(f"- Interpolation enabled: {processor.interpolation_enabled}")
    print("- Smooth timeline with no gaps")
    print("- Performance optimized for real-time playback")
    print()

    improvement_ratio = (
        config["video_processing"]["frame_skip"] / processor.dense_frame_interval
    )
    print(
        f"IMPROVEMENT: {improvement_ratio}x more frames processed for smoother real-time experience"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "1. Run dense processing on a video: python src/services/realtime_video_processor.py <video_path>"
        )
        print(
            "2. Compare the generated *_dense_metadata.json with existing *_metadata.json files"
        )
        print(
            "3. Use the video player frontend to see smooth real-time face gallery synchronization"
        )
