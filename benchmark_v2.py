...
try:
    ...
    print("Please check that all required modules are installed and you're running the script from the project root.")
    sys.exit(1)
...
def benchmark_video_processing(video_path, std_embeddings, chroma_recognizer, std_recognizer, detector, frame_limit=100):
    ...
    if HAS_CHROMADB and chroma_recognizer is not None:
        ...
        for count in embedding_counts:
            if count <= len(selected_contestants):
                ...
                subset_std_embeddings = {k: std_embeddings[k] for k in subset if k in std_embeddings}
                ...
                if test_results and test_results["chromadb"]:
                    speedup = test_results["standard"]["avg_time"] / test_results["chromadb"]["avg_time"]
