...
def overlay_faces(image, faces, matches, font_path=None):
    ...
    for i, (face, match_list) in enumerate(zip(faces, matches, strict=False)):
...
def compare_recognition(image_path, threshold_original=0.35, threshold_fixed=0.6):
    ...
    recognizer_fixed = FaceRecognizer(
        face_detector=detector, similarity_threshold=threshold_fixed, use_arcface=True
    )
    ...
    for face in detected_faces:
        ...
        if top_matches:
            logger.info(
                f"Original - Face {len(original_matches)}: Top match {top_matches[0][0]} with confidence {top_matches[0][1]:.4f}"
            )
        else:
            logger.info(
                f"Original - Face {len(original_matches)}: No matches above threshold {threshold_original}"
            )
    ...
    for face in detected_faces:
        ...
        if top_matches:
            logger.info(
                f"Fixed - Face {len(fixed_matches)}: Top match {top_matches[0][0]} with confidence {top_matches[0][1]:.4f}"
            )
        else:
            logger.info(
                f"Fixed - Face {len(fixed_matches)}: No matches above threshold {threshold_fixed}"
            )

    # Generate visualizations
    original_img = overlay_faces(
        image_rgb.copy(),
        detected_faces,
