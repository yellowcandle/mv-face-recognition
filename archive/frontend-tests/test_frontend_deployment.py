...
class FrontendTester:
    ...
    async def test_api_endpoints(
        ...
        for endpoint in endpoints:
            ...
            try:
                ...
                results[endpoint] = {"url": url, "success": False, "error": str(e)}
    ...
    async def test_backend_direct(
        ...
        endpoints = ["/", "/health", "/api/system/info"]
    ...
    async def test_static_assets(
        ...
        for asset in assets:
            ...
            try:
                ...
                results[asset] = {"url": url, "success": False, "error": str(e)}
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            self.results["tests"]["frontend_routes"] = await self.test_frontend_routes(
                session
            )
            ...
            self.results["tests"]["backend_direct"] = await self.test_backend_direct(
                session
            )

            print("Testing static assets...")
            self.results["tests"]["static_assets"] = await self.test_static_assets(
                session
            )
    ...
    def generate_report(self) -> str:
        ...
        avg_load_time = (
            sum(r.get("load_time_ms", 0) for r in frontend_routes.values())
            / total_routes
        )
        ...
        report.append(
            f"- **Success Rate**: {(successful_routes/total_routes)*100:.1f}%"
        )
        ...
        failed_routes = [
            route for route, result in frontend_routes.items() if not result["success"]
        ]
        if failed_routes:
        ...
        slow_routes = [
            route
            for route, result in frontend_routes.items()
            if result.get("load_time_ms", 0) > 5000
        ]
        if slow_routes:
...
async def main():
    ...
    with open(
        "/Users/swong/dev/mv-face-recognition/frontend_test_results.json", "w"
    ) as f:
