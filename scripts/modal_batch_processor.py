...
class ModalProcessor:
    ...
    def __enter__(self):
        ...
        config["paths"]["processed_videos_dir"] = str(
    ...
    def run_batch_processing(
        ...
        try:
            ...
            if single_video:
                ...
                results = self.processor.batch_process_all_videos(
                    force_reprocess, overall_progress_callback
                )
            ...
            print("=" * 60)
            print(
                "Generating 6x more timeline data for smooth real-time synchronization..."
            )

            (
                self._generate_dense_metadata_for_processed_videos()
            )
    ...
    def _generate_dense_metadata_for_processed_videos(self):
        ...
        from src.services.realtime_video_processor import (
            create_dense_metadata_for_video,
        )
        ...
        if not processed_videos_dir.exists():
            print(
                "⚠️ No processed videos directory found, skipping dense metadata generation"
            )
        ...
        print(
            f"Found {len(processed_videos)} processed videos for dense metadata generation"
        )
        ...
        for video_path in processed_videos:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    print(
                        f"   👥 Contestants detected: {len(result['contestants_detected'])}"
                    )
                else:
                    dense_results[video_name] = {
                        "success": False,
                    ...
                    print(
                        f"❌ Failed to generate dense metadata: {result.get('error', 'Unknown error')}"
                    )

            except Exception as e:
                dense_results[video_name] = {"success": False, "error": str(e)}
                print(f"❌ Exception generating dense metadata for {video_name}: {e}")

        # Summary
        successful_dense = len(
            [r for r in dense_results.values() if r.get("success", False)]
        )
        ...
        if successful_dense > 0:
            ...
            print(
                "   Videos now have 6x more timeline data for smooth video player synchronization"
            )
...
def main(
    ...
    if dry_run:
        print(
            "Note: --dry-run is not applicable for Modal execution and will be ignored."
        )
        print(
            "The script will connect to Modal but not run the job if you wish to proceed."
        )

    # Validate similarity threshold locally
    if not 0.0 <= similarity_threshold <= 1.0:
        print(
            "Error: Similarity threshold must be between 0.0 and 1.0", file=sys.stderr
        )
        sys.exit(1)
