        ...
        if faces:
            ...
            for i, face_data in enumerate(faces):
                ...
                if results["ids"] and results["ids"][0]:
                    for j, (name, distance) in enumerate(
                        zip(results["ids"][0], results["distances"][0])
                    ):
                        similarity = 1.0 - distance
                        print(
                            f"      {j+1:2d}. {name:15s}: {similarity:.4f} (distance: {distance:.4f})"
                        )
...
def main():
    ...
    print("\n💡 RECOMMENDATIONS:")
    print(
        "   - If all similarities are very low (< 0.1), there may be an embedding format issue"
    )
