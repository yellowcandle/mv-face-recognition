...
class EmbeddingDeduplicator:
    ...
    def analyze_database(self) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                "unique_embeddings": len(ids) - len(exact_duplicates) - len(similar_duplicates)
            }
    ...
    def _find_exact_duplicates(self, ids: List[str], embeddings: List[np.ndarray]) -> List[Tuple[str, str, float]]:
        ...
        for i in range(len(embeddings)):
            for j in range(i + 1, len(embeddings)):
                ...
                if similarity >= self.similarity_threshold_exact:
                    ...
                    logger.debug(f"Exact duplicate found: {ids[i]} ↔ {ids[j]} (similarity: {similarity:.6f})")
        
        logger.info(f"Found {len(exact_duplicates)} exact duplicate pairs")
        return exact_duplicates

    def _find_similar_duplicates(self, ids: List[str], embeddings: List[np.ndarray]) -> List[Tuple[str, str, float]]:
        ...
        for i in range(len(embeddings)):
            for j in range(i + 1, len(embeddings)):
                ...
                if self.similarity_threshold_similar <= similarity < self.similarity_threshold_exact:
                    similar_duplicates.append((ids[i], ids[j], similarity))
                    logger.debug(f"Similar embedding found: {ids[i]} ↔ {ids[j]} (similarity: {similarity:.6f})")
    ...
    def remove_duplicates(self, analysis: Dict, mode: str = "exact") -> int:
        ...
        logger.info(f"🗑️ Removing duplicates in mode: {mode} (prioritizing names over numbers)")
        ...
        if mode in ["exact", "all"]:
            ...
            for id1, id2, similarity in analysis["exact_duplicates"]:
                ...
                reason = ""
                if self._is_contestant_number(remove_id) and not self._is_contestant_number(keep_id):
                    reason = " (prioritized name over number)"
                elif not self._is_contestant_number(remove_id) and self._is_contestant_number(keep_id):
                    reason = " (prioritized name over number)"
                else:
                    reason = " (kept first occurrence)"
                
                logger.info(f"Marking {remove_id} for removal, keeping {keep_id}{reason}")
        ...
        if mode in ["similar", "all"]:
            ...
            for id1, id2, similarity in analysis["similar_duplicates"]:
                ...
                logger.info(f"Marking {remove_id} for removal, keeping {keep_id}{reason} (similarity: {similarity:.4f})")
        ...
        if analysis["duplicate_ids"]:
            ...
            for duplicate_id in analysis["duplicate_ids"]:
                ...
                occurrences = [i for i, id_ in enumerate(all_data["ids"]) if id_ == duplicate_id]
                # Mark all but the first for removal
                for i in occurrences[1:]:
                    ids_to_remove.add(all_data["ids"][i])
                    logger.info(f"Marking duplicate occurrence of {duplicate_id} for removal")
        ...
        if ids_to_remove:
            try:
                ...
                logger.info(f"Successfully removed {len(ids_to_remove)} duplicate embeddings")
                
                # Log summary of what was kept
                names_kept = [id_ for id_ in kept_ids if not self._is_contestant_number(id_)]
                numbers_kept = [id_ for id_ in kept_ids if self._is_contestant_number(id_)]
                logger.info(f"Summary: Kept {len(names_kept)} names and {len(numbers_kept)} contestant numbers")
    ...
    def standardize_to_names(self) -> Dict:
        ...
        logger.info(f"Current database state: {name_count} names, {number_count} contestant numbers")
        
        return {
            "total": len(ids),
            "names": name_count,
            "numbers": number_count,
            "consistency": "Good" if number_count == 0 else "Mixed" if name_count > number_count else "Poor"
        }
    ...
    def print_analysis_report(self, analysis: Dict):
        ...
        if analysis["exact_duplicates"]:
            ...
            for i, (id1, id2, similarity) in enumerate(analysis["exact_duplicates"][:10]):
                keep_id, remove_id = self._choose_better_id(id1, id2)
                priority_reason = ""
                if self._is_contestant_number(remove_id) and not self._is_contestant_number(keep_id):
                ...
                print(f"  {i+1:2d}. {id1} ↔ {id2} (similarity: {similarity:.6f}){priority_reason}")
        ...
        if analysis["similar_duplicates"]:
            ...
            print("     [Will prioritize names over contestant numbers]")
            for i, (id1, id2, similarity) in enumerate(analysis["similar_duplicates"][:10]):
                keep_id, remove_id = self._choose_better_id(id1, id2)
                priority_reason = ""
                if self._is_contestant_number(remove_id) and not self._is_contestant_number(keep_id):
                    priority_reason = " → Keep name"
        ...
        total_duplicates = len(analysis["exact_duplicates"]) + len(analysis["similar_duplicates"])
...
def main():
    ...
    parser.add_argument('--analyze', action='store_true', help='Analyze database for duplicates')
    parser.add_argument('--remove', choices=['exact', 'similar', 'all'], 
                        help='Remove duplicates: exact, similar, or all')
    parser.add_argument('--optimize', action='store_true', help='Optimize database after deduplication')
    parser.add_argument('--standardize', action='store_true', help='Show database standardization info')
    parser.add_argument('--dry-run', action='store_true', help='Show what would be removed without actually removing')
    parser.add_argument('--exact-threshold', type=float, default=0.999,
                        help='Similarity threshold for exact duplicates (default: 0.999)')
    parser.add_argument('--similar-threshold', type=float, default=0.95,
    ...
    if args.remove and not args.dry_run:
        ...
        names_to_remove = [id_ for id_ in ids_to_remove if not deduplicator._is_contestant_number(id_)]
        numbers_to_remove = [id_ for id_ in ids_to_remove if deduplicator._is_contestant_number(id_)]
