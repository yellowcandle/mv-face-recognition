...
class EmbeddingGenerator:
    ...
    def __init__(
        self,
        photo_dir: str = "source/photo/contestants",
    ...
    def extract_face_encoding(
        self, image_path: Path, max_faces: int = 1
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if len(face_locations) > max_faces:
                logger.warning(
                    f"Multiple faces found in {image_path}, using the first one"
                )
    ...
    def generate_average_embedding(self, contestant_id: str) -> Optional[np.ndarray]:
        ...
        logger.info(
            f"Generated average embedding for contestant {contestant_id} from {len(encodings)} photos"
        )
    ...
    def regenerate_all_embeddings(self, force_overwrite: bool = False):
        ...
        for contestant_id in self.contestants_info.keys():
            try:
                ...
                if existing_embedding.exists() and not force_overwrite:
                    logger.info(
                        f"Skipping {nickname} (embedding exists, use --force to overwrite)"
                    )
                    continue
        ...
        logger.info(
            f"Embedding generation complete: {success_count} success, {error_count} errors"
        )
    ...
    def regenerate_single_embedding(self, contestant_id: str):
        ...
        try:
            ...
            if embedding is not None:
                self.save_embedding(contestant_id, embedding)
                logger.info(
                    f"Successfully regenerated embedding for contestant {contestant_id}"
                )
                return True
            else:
                logger.error(
            ...
            logger.error(
                f"Error regenerating embedding for contestant {contestant_id}: {e}"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Regenerate face embeddings for contestants"
    )
    parser.add_argument(
        "--contestant-id",
        "-c",
    ...
    parser.add_argument(
        "--force", "-f", action="store_true", help="Force overwrite existing embeddings"
    )
    parser.add_argument(
        "--all", "-a", action="store_true", help="Regenerate all embeddings"
    )
    ...
    if args.contestant_id:
        ...
        print(
            "  Regenerate specific contestant: python generate_embeddings.py --contestant-id 1"
        )
