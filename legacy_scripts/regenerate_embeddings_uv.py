...
class EmbeddingRegenerator:
    ...
    def extract_face_embedding_enhanced(self, image_path: Path) -> Optional[np.ndarray]:
        ...
        try:
            ...
            detections = self.face_detector.detect_faces(
                frame, timestamp=0.0, frame_number=0
            )
            ...
            if (
                hasattr(face_detection, "embedding")
                and face_detection.embedding is not None
            ):
    ...
    def generate_embedding_for_contestant(
        self, contestant_id: str
    ) -> Optional[np.ndarray]:
        ...
        if not embeddings:
            logger.error(
                f"No valid embeddings generated for contestant {contestant_id}"
            )
            return None
        ...
        logger.info(
            f"Generated embedding for contestant {contestant_id} from {len(embeddings)} photos"
        )
    ...
    def save_embedding(self, contestant_id: str, embedding: np.ndarray):
        ...
        if saved_count > 0:
            logger.info(
                f"✅ Saved embedding for {nickname} (ID: {contestant_id}) to {saved_count} files"
            )
            return True
        else:
            logger.error(
                f"❌ Failed to save embedding for {nickname} (ID: {contestant_id})"
            )
            return False
    ...
    def regenerate_all_embeddings(self, force: bool = False):
        ...
        for contestant_id, info in self.contestant_db.contestants_info.items():
            ...
            if existing_path.exists() and not force:
                logger.info(
                    f"⏭️  Skipping {nickname} (exists, use --force to overwrite)"
                )
                skip_count += 1
                continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Regenerate face embeddings using UV environment"
    )
    parser.add_argument("--all", action="store_true", help="Regenerate all embeddings")
    parser.add_argument(
        "--force", action="store_true", help="Force overwrite existing embeddings"
    )
    parser.add_argument(
        "--contestant-id", type=str, help="Regenerate embedding for specific contestant"
    )
    ...
    try:
        ...
        if args.contestant_id:
            ...
            success, skip, error = regenerator.regenerate_all_embeddings(
                force=args.force
            )
