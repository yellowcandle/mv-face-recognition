...
class ChromaDBOptimizer:
    ...
    def get_thread_local_client(self):
        ...
        if not hasattr(self.thread_local, "client"):
            ...
            self.thread_local.collection = self.thread_local.client.get_collection(name=self.collection_name)
    ...
    def batch_add_embeddings(
        ...
        try:
            # Process in batches
            batches = [embeddings_data[i:i + self.batch_size] 
    ...
    def parallel_query(
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_embedding = {executor.submit(_execute_query, emb): i 
            ...
            logger.info(f"Completed {len(query_embeddings)} parallel queries in {total_time:.3f} seconds")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "query_times": {
                ...
                "average": sum(self.perf_stats["query_times"]) / max(1, len(self.perf_stats["query_times"])),
            ...
            "batch_add_times": {
                ...
                "average": sum(self.perf_stats["batch_add_times"]) / max(1, len(self.perf_stats["batch_add_times"])),
                "min": min(self.perf_stats["batch_add_times"]) if self.perf_stats["batch_add_times"] else 0,
                "max": max(self.perf_stats["batch_add_times"]) if self.perf_stats["batch_add_times"] else 0
            },
            "memory_usage": {
                "count": len(self.perf_stats["memory_usage"]),
                "average": sum(self.perf_stats["memory_usage"]) / max(1, len(self.perf_stats["memory_usage"])),
                "min": min(self.perf_stats["memory_usage"]) if self.perf_stats["memory_usage"] else 0,
                "max": max(self.perf_stats["memory_usage"]) if self.perf_stats["memory_usage"] else 0,
