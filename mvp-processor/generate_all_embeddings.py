    ...
    try:
        ...
        if contestant_id:
            ...
            contestants_to_process = contestants_df[
                contestants_df["編號"] == contestant_id
            ]
        ...
        with tqdm(total=stats["total"], desc="Processing contestants") as pbar:
            for _, row in contestants_to_process.iterrows():
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to process contestant {contestant_id} ({nickname}): {e}"
                    )
                    stats["errors"] += 1
...
def process_contestant(
        ...
        if validate_only:
            ...
            try:
                ...
                if metadata.get("method") == expected_method and len(embedding) == 512:
                    logger.debug(
                        f"Validated embedding for contestant {contestant_id} ({nickname})"
                    )
                    return "skipped"
                else:
                    logger.warning(
                        f"Invalid embedding for contestant {contestant_id}: method={metadata.get('method')}, dim={len(embedding)}"
                    )
                    return "error"
            except Exception as e:
                logger.error(
                    f"Failed to validate embedding for contestant {contestant_id}: {e}"
                )
                return "error"
        else:
            logger.debug(
                f"Embedding already exists for contestant {contestant_id} ({nickname})"
            )
            return "skipped"
    ...
    if not contestant_dir.exists():
        logger.error(
            f"Photo directory not found for contestant {contestant_id}: {contestant_dir}"
        )
        return "error"
    ...
    if not photo_path:
        logger.error(
            f"No primary photo found for contestant {contestant_id} in {contestant_dir}"
        )
        return "error"
    ...
    try:
        # Generate embedding
        logger.debug(
            f"Generating embedding for contestant {contestant_id} from {photo_path}"
        )
        ...
        logger.error(
            f"Failed to generate embedding for contestant {contestant_id}: {e}"
        )
        return "error"
...
def validate_all_embeddings(output_dir, contestants_df):
    ...
    for _, row in contestants_df.iterrows():
        ...
        embedding_path = (
            output_dir / f"contestant_{contestant_id}_unified_embedding.npy"
        )
        metadata_path = (
            output_dir / f"contestant_{contestant_id}_embedding_metadata.json"
        )
        ...
        if embedding_path.exists() and metadata_path.exists():
            try:
                ...
                if len(embedding) == 512 and metadata.get("method") in [
                    "insightface",
                    "face_recognition",
                    "opencv_custom",
                ]:
                    ...
                    invalid_count += 1
                    click.echo(
                        f"   ❌ Invalid: {contestant_id} ({nickname}) - dim={len(embedding)}, method={metadata.get('method')}"
                    )
            except Exception as e:
                invalid_count += 1
