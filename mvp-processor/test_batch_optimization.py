...
def create_test_frames(num_frames: int = 8, frame_size: tuple = (480, 640, 3)) -> list:
    ...
    for i in range(num_frames):
        ...
        for _ in range(num_faces):
            ...
            cv2.rectangle(frame, (x, y), (x + face_size, y + face_size), (200, 180, 160), -1)
            ...
            cv2.ellipse(frame, (x + face_size // 2, mouth_y), (15, 8), 0, 0, 180, (100, 50, 50), -1)
...
def test_individual_vs_batch_performance():
    ...
    logger.info(f"Individual processing: {individual_time:.3f}s, {individual_faces} faces detected")
    ...
    try:
        batch_results = unified_detector.detect_faces_batch(test_frames, timestamps, frame_numbers)
        batch_time = time.time() - start_time
        batch_faces = sum(len(detections) for detections in batch_results)
        
        logger.info(f"Batch processing: {batch_time:.3f}s, {batch_faces} faces detected")
...
def test_gpu_memory_manager():
    ...
    try:
        ...
        logger.info(f"Initialized GPU Memory Manager on device: {memory_manager.device}")
        
        # Get initial memory info
        initial_info = memory_manager.get_memory_info()
        logger.info(f"Initial memory usage: {initial_info.get('current_usage_gb', 0):.2f} GB")
        ...
        if memory_manager.device in ["cuda", "mps"]:
            ...
            logger.info(f"Memory usage after allocation: {after_alloc_info.get('current_usage_gb', 0):.2f} GB")
            ...
            logger.info(f"Final memory usage: {final_info.get('current_usage_gb', 0):.2f} GB")
...
def test_enhanced_detector_batch():
    ...
    try:
        ...
        logger.info(f"Initialized Enhanced Face Detector with backend: {enhanced_detector.backend_type}")
        ...
        logger.info(f"Enhanced detector batch processing: {batch_time:.3f}s, {total_faces} faces")
        ...
        logger.info("This is expected if InsightFace is not installed or hardware acceleration is not available")
