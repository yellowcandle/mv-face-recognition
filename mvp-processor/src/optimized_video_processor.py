...
class MemoryManager:
    ...
    def memory_context(self, operation_name: str):
        ...
        logger.debug(
            f"Starting {operation_name} - Memory: {start_stats.current_mb:.1f}MB"
        )
        ...
        try:
            ...
            logger.debug(
                f"Completed {operation_name} - Memory delta: {memory_used:+.1f}MB"
            )
...
class EmbeddingCache:
    ...
    def get(
        self, face_region: np.ndarray, trajectory_id: str = None
    ) -> Optional[np.ndarray]:
    ...
    def put(
        self, face_region: np.ndarray, embedding: np.ndarray, trajectory_id: str = None
    ):
...
class StreamingFrameExtractor:
    ...
    def __init__(
        self, fps_sample_rate: float = 6, batch_size: int = 8, max_frames: int = 0
    ):
    ...
    def extract_batches(
        self, video_path: str
    ) -> Generator[ProcessingBatch, None, None]:
        ...
        try:
            ...
            logger.info(
                f"Extracting frames in batches of {self.batch_size} from {video_path}"
            )
            logger.info(
                f"Frame interval: {frame_interval} (every {frame_interval / fps:.3f}s)"
            )
...
class ParallelBatchProcessor:
    ...
    def process_batch_parallel(self, batch: ProcessingBatch, process_func, **kwargs):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_frame = {
                executor.submit(
                    process_func, frame, timestamp, idx + batch.start_idx, **kwargs
                ): (frame, timestamp, idx + batch.start_idx)
...
class OptimizedVideoProcessor:
    ...
    def process_video_optimized(
        ...
        with self.memory_manager.memory_context("video_processing"):
            ...
            logger.info(
                f"Processing {total_frames} frames in batches of {self.batch_size}"
            )
    ...
    def _process_batch_sequential(
        self, batch: ProcessingBatch, face_detector, face_recognizer, face_tracker
    ) -> List[Any]:
    ...
    def _process_single_frame(
        self, frame, timestamp, frame_idx, face_detector, face_recognizer, face_tracker
    ):
    ...
    def _log_performance_summary(self, total_frames: int, total_time: float):
        ...
        memory_improvement = (
            1 - memory_stats.peak_mb / original_estimated_memory
        ) * 100
...
class StreamingVideoProcessorAdapter:
    ...
    def get_performance_stats(self):
        ...
        if self.optimized:
            ...
            return {
                "optimized": False,
                "message": "Standard processor has no performance stats",
            }
