...
def test_embedding_consistency():
    ...
    print(
        f"Embedding dimension: {embedding_system.embedding_config.embedding_dimension}"
    )
    print(
        f"Normalization enabled: {embedding_system.embedding_config.normalize_embeddings}"
    )
    ...
    euclidean_dist = embedding_system.calculate_distance(
        embedding, embedding2, "euclidean"
    )
    ...
    euclidean_conf = embedding_system.distance_to_confidence(
    ...
    diff_cosine_dist = embedding_system.calculate_distance(
        embedding, embedding3, "cosine"
    )
    diff_cosine_conf = embedding_system.distance_to_confidence(
        diff_cosine_dist, "cosine"
    )
...
def test_unified_detector():
    ...
    try:
        ...
        print(
            f"   Embedding method: {unified_detector.embedding_system.embedding_config.method.value}"
        )
        ...
        if db_stats["unified_count"] < db_stats["embeddings_loaded"]:
            print(
                f"   ⚠️ {db_stats['embeddings_loaded'] - db_stats['unified_count']} embeddings need migration"
            )
        ...
        if len(detections) > 0:
            ...
            for i, recognition in enumerate(recognitions[:3]):  # Show first 3
                print(
                    f"      {i + 1}. {recognition.contestant_nickname} "
...
def test_migration_analysis():
    ...
    photo_dir = Path(
        config.get("contestants", {}).get("photo_dir", "../source/photo/contestants")
    )
    ...
    if len(legacy_files) > 0:
        print(
            f"   1. Run migration: python migrate_embeddings.py --config {config_path}"
        )

    if len(unified_files) < len(legacy_files):
        print("   2. Consider regenerating from photos for better quality:")
        print(
            f"      python migrate_embeddings.py regenerate-from-photos --config {config_path}"
        )

    print("   3. Update config to enable unified system:")
    print("      face_detection:")
    print("        use_unified_system: true")

    print("   4. Test with sample video to verify recognition accuracy")
...
def main():
    ...
    try:
        ...
        if unified_detector is None:
            ...
            print(
                "   Run 'python migrate_embeddings.py --stats-only' to see migration status"
            )
