...
class MetadataGenerator:
    ...
    def generate_metadata(self, video_info: Dict, recognitions: List[FaceRecognition], 
                         frame_data: List[Dict], output_name: str) -> Dict:
        """
        ...
        timeline_markers = self._create_timeline_markers(recognitions, video_info['duration'])
    ...
    def _build_contestant_timeline(self, recognitions: List[FaceRecognition]) -> Dict:
        ...
        for recognition in recognitions:
            ...
            # Add appearance
            appearance = {
                'timestamp': recognition.detection.timestamp,
                'frame_number': recognition.detection.frame_number,
                'confidence': recognition.match_confidence,
                'face_location': recognition.detection.location
            }
    ...
    def _generate_processing_summary(self, recognitions: List[FaceRecognition], 
                                   frame_data: List[Dict]) -> Dict:
        ...
        # Recognition rate
        recognition_rate = total_recognitions / total_faces_detected if total_faces_detected > 0 else 0
    ...
    def _create_timeline_markers(self, recognitions: List[FaceRecognition], 
        ...
        for interval, interval_recognitions in intervals.items():
            ...
            marker = {
                ...
                'average_confidence': sum(r.match_confidence for r in interval_recognitions) / len(interval_recognitions)
            }
            markers.append(marker)
        
        return sorted(markers, key=lambda x: x['start_time'])
        
    def generate_face_galleries(self, recognitions: List[FaceRecognition], 
        ...
        for contestant_id, gallery in galleries.items():
            contestant_recognitions = [r for r in recognitions if r.contestant_id == contestant_id]
            ...
            gallery['statistics'] = {
                ...
                'screen_time_percentage': self._calculate_screen_time_percentage(
                    timestamps, 
                    recognitions[0].detection.timestamp if recognitions else 0  # Total duration proxy
                )
            }
    ...
    def _calculate_screen_time_percentage(self, timestamps: List[float], 
