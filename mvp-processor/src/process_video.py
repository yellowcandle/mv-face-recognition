...
class VideoProcessingPipeline:
    ...
    def setup_output_dirs(self):
        ...
        for dir_key in ['processed_dir', 'thumbnails_dir', 'metadata_dir', 'galleries_dir']:
    ...
    def initialize_database(self, force_rebuild: bool = False):
        ...
        logger.info(f"Database ready with {len(self.contestant_db.face_encodings)} contestants")
    ...
    def process_video(self, video_path: str, output_name: str = None) -> Dict:
        ...
        # Create thumbnail
        thumbnail_path = Path(self.config['output']['thumbnails_dir']) / f"{output_name}_thumb.jpg"
        ...
        for frame_idx, (frame, timestamp) in enumerate(tqdm(frames_list, desc="Processing frames")):
            # Preprocess frame
                    ...
                    'recognitions': [
                        {
                            ...
                            'face_location': r.detection.location
                        }
        ...
        # Save metadata
        metadata_path = Path(self.config['output']['metadata_dir']) / f"{output_name}_metadata.json"
...
@click.option('--config', '-c', default='config/processing_config.yaml', help='Configuration file')
@click.option('--output-name', '-o', help='Custom output name (default: video filename)')
...
def main(input: str, config: str, output_name: str, no_upload: bool, rebuild_db: bool, debug: bool):
    ...
    try:
        ...
        print(f"   Faces detected: {metadata['processing_summary']['total_faces_detected']}")
        ...
        if not no_upload:
            print(f"   ☁️  Uploaded to Cloudflare R2")
