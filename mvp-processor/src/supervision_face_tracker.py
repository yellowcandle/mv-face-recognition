...
class SupervisionFaceTrajectory:
    ...
    bounding_boxes: List[Tuple[float, float, float, float]] = field(default_factory=list)  # xyxy format
...
class SupervisionFaceTracker:
    ...
    def __init__(self, config: dict):
        ...
        frame_rate = self.detection_sampling_rate  # Use face detection rate, not video rate
        ...
        face_optimized_buffer = min(self.max_trajectory_gap, 8)  # Limit buffer for efficiency
        ...
        logger.info(f"Optimized SupervisionFaceTracker initialized with ByteTracker")
        logger.info(f"Parameters: frame_rate={frame_rate}, activation={face_optimized_activation}, "
                   f"buffer={face_optimized_buffer}, matching={face_optimized_matching}")
        logger.info(f"Adaptive tracking: {self.enable_adaptive_tracking}, "
                   f"Single face optimization: {self.single_face_optimization}")
    
    def convert_detections_to_supervision(self, detections: List[FaceDetection]) -> sv.Detections:
    ...
    def convert_supervision_to_trajectory_mapping(self, 
                                                 sv_detections: sv.Detections, 
                                                 original_detections: List[FaceDetection]) -> Dict[int, FaceDetection]:
    ...
    def update_trajectories(self, detections: List[FaceDetection], recognitions: List[FaceRecognition]) -> List[SupervisionFaceTrajectory]:
        ...
        tracker_to_detection_map = self.convert_supervision_to_trajectory_mapping(tracked_detections, detections)
        ...
        if hasattr(tracked_detections, 'tracker_id'):
            for tracker_id in tracked_detections.tracker_id:
                ...
                if trajectory_id in self.active_trajectories:
                    self.active_trajectories[trajectory_id].add_detection(detection, recognition, self.tracking_config)
    ...
    def _handle_single_face_tracking(self, detection: FaceDetection, recognitions: List[FaceRecognition]) -> List[SupervisionFaceTrajectory]:
