...
@click.option("--stats-only", "-s", is_flag=True, help="Only show migration statistics")
def migrate_embeddings(config, method, force, dry_run, stats_only):
    ...
    try:
        ...
        if method:
            ...
            logger.info(
                f"Using configured embedding method: {embedding_system.embedding_config.method.value}"
            )
        ...
        click.echo(
            f"   Distance threshold: {system_stats['config']['distance_threshold']:.3f}"
        )
        ...
        if not dry_run and stats["migrated"] > 0:
            click.echo("\n🎯 Next Steps:")
            click.echo("   1. Update your config.yaml to enable unified system:")
            click.echo("      face_detection:")
            click.echo("        use_unified_system: true")
            click.echo("   2. Test the system with a sample video:")
            click.echo("      python process_video.py --input sample_video.mp4")
            click.echo(
                "   3. Monitor recognition accuracy and adjust thresholds if needed"
            )
...
def _show_current_state(embeddings_dir: Path, embedding_system: UnifiedEmbeddingSystem):
    ...
    if needs_migration == 0:
        click.echo(
            "   ✅ All embeddings are already in unified format with target method"
        )
    else:
        click.echo(
            f"   ⚠️  {needs_migration} embeddings need migration to {target_method}"
        )


@click.command()
@click.option("--config", "-c", default="config.yaml", help="Configuration file path")
@click.option("--contestant-photos", "-p", help="Path to contestant photos directory")
@click.option(
...
def regenerate_from_photos(config, contestant_photos, force):
    ...
    try:
        ...
        # Initialize system
        from unified_face_detector import (
            UnifiedContestantDatabase,
        ...
        if not force:
            click.confirm(
                "This will regenerate embeddings from photos. Continue?", abort=True
            )
        ...
        if stats["errors"] > 0:
            ...
            click.echo(
                "   Make sure all contestant photos exist in the specified directory"
            )
