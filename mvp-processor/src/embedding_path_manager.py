...
class EmbeddingPathManager:
    ...
    def get_embedding_paths(
        self, contestant_id: str, nickname: str
    ) -> Tuple[Path, Path]:
        """
    ...
    def find_existing_embedding_paths(
        self, contestant_id: str, nickname: str
        ...
        legacy_patterns = [
            (
                f"{contestant_id}_embedding.npy",
                f"{contestant_id}_embedding_metadata.json",
            ),
        ...
        for emb_pattern, meta_pattern in legacy_patterns:
            ...
            if emb_path.exists():
                ...
                logger.debug(
                    f"Found legacy embedding pattern for {nickname}: {emb_pattern}"
                )
    ...
    def get_photo_paths(
        self, contestant_id: str, nickname: str, name: str
    ) -> List[Path]:
        """
    ...
    def migrate_to_standard_naming(self, contestant_id: str, nickname: str) -> bool:
        ...
        # Get standard paths
        standard_emb_path, standard_meta_path = self.get_embedding_paths(
        ...
        # Find existing files
        existing_emb_path, existing_meta_path = self.find_existing_embedding_paths(
            contestant_id, nickname
        )
        ...
        try:
            ...
            if not standard_emb_path.exists():
                ...
                logger.info(
                    f"Migrated embedding for {nickname}: {existing_emb_path.name} -> {standard_emb_path.name}"
                )
            ...
            if (
                ...
                and not standard_meta_path.exists()
            ):
                existing_meta_path.rename(standard_meta_path)
                logger.info(
                    f"Migrated metadata for {nickname}: {existing_meta_path.name} -> {standard_meta_path.name}"
                )
    ...
    def validate_embedding_consistency(self) -> Dict[str, any]:
        ...
        embedding_files = list(self.photo_dir.glob("*_embedding.npy")) + list(
            self.photo_dir.glob("*_unified_embedding.npy")
        )
        ...
        for emb_file in embedding_files:
            ...
            if "unified_embedding" in emb_file.name:
                meta_file = emb_file.with_name(
                    emb_file.name.replace(
                        "_unified_embedding.npy", "_embedding_metadata.json"
                    )
                )
            else:
                meta_file = emb_file.with_name(
                    emb_file.name.replace("_embedding.npy", "_embedding_metadata.json")
                )
