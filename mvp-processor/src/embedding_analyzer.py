...
class EmbeddingAnalyzer:
    ...
    def load_all_embeddings(self, force_reload: bool = False) -> Dict[str, np.ndarray]:
        ...
        for contestant_id, info in self.contestants_info.items():
            ...
            embedding_paths = [
                ...
                Path(f"../../source/photo/contestants/{nickname}_embedding.npy"),
                Path(f"../source/photo/contestants/contestant_{contestant_id}_unified_embedding.npy"),
                Path(f"../../source/photo/contestants/contestant_{contestant_id}_unified_embedding.npy"),
            ]
            ...
            for embedding_path in embedding_paths:
                try:
                    if embedding_path.exists():
                        ...
                        logger.debug(f"Loaded {nickname}: shape {embedding.shape}, norm {np.linalg.norm(embedding):.3f}")
                        break
                except Exception as e:
    ...
    def analyze_distance_distribution(self, embeddings: Dict[str, np.ndarray] = None) -> Dict[str, Any]:
    ...
    def analyze_embedding_quality(self, embeddings: Dict[str, np.ndarray] = None) -> Dict[str, Any]:
        ...
        if len(norms) > 5:  # Need sufficient data for outlier analysis
            ...
            for name, embedding in embeddings.items():
                ...
                if norm < lower_bound or norm > upper_bound:
                    outliers.append({"name": name, "norm": float(norm), "type": "norm_outlier"})
    ...
    def compare_backend_embeddings(self) -> Dict[str, Any]:
        ...
        for contestant_id, info in self.contestants_info.items():
            ...
            possible_files = [
                ...
                base_path / f"contestant_{contestant_id}_face_recognition_embedding.npy",
            ...
            for name, backends in backend_embeddings.items():
                ...
                for i, backend1 in enumerate(backend_names):
                    for backend2 in backend_names[i+1:]:
                        ...
                        consistency_scores[f"{backend1}_vs_{backend2}"] = float(cosine_sim)
                
                comparison_results["cross_backend_consistency"][name] = consistency_scores
    ...
    def _generate_recommendations(self, embeddings: Dict[str, np.ndarray]) -> List[str]:
        ...
        if len(embeddings) < len(self.contestants_info) * 0.8:
            missing_percentage = (1 - len(embeddings) / len(self.contestants_info)) * 100
    ...
    def visualize_embedding_space(self, embeddings: Dict[str, np.ndarray] = None, 
                                method: str = "tsne", output_file: str = None):
        ...
        if method.lower() == "tsne":
            reducer = TSNE(n_components=2, random_state=42, perplexity=min(30, len(names)//2))
        ...
        scatter = plt.scatter(coords[:, 0], coords[:, 1], alpha=0.7, s=50)
    ...
    def export_embeddings_for_analysis(self, format: str = "csv", output_file: str = None):
        ...
        if format.lower() == "csv":
            ...
            data = []
            for name, embedding in embeddings.items():
                row = {"name": name, **{f"dim_{i}": val for i, val in enumerate(embedding.flatten())}}
                data.append(row)
...
if CLICK_AVAILABLE:
    @click.command()
    @click.option('--mode', type=click.Choice(['quick', 'comprehensive', 'visual']), 
    ...
    def main(mode, output, output_format):
        ...
        if mode == "quick":
            ...
            print(f"   Valid embeddings: {quality_analysis['validity_analysis']['valid_embeddings']}")
            
            if distance_analysis.get('distance_metrics'):
                cosine_stats = distance_analysis['distance_metrics']['cosine']
                print(f"   Cosine distance range: {cosine_stats['min']:.3f} - {cosine_stats['max']:.3f}")
            ...
            if output:
                ...
                print(f"   Total contestants: {report['metadata']['total_contestants']}")
                print(f"   Analyzed embeddings: {report['metadata']['analyzed_embeddings']}")
