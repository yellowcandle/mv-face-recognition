...
class UnifiedEmbeddingSystem:
    ...
    def _load_embedding_config(self, config: dict) -> EmbeddingConfig:
        ...
        if face_detection_config.get("enable_hardware_acceleration") and face_detection_config.get("model") == "insightface":
            method = EmbeddingMethod.INSIGHTFACE
            distance_threshold = 0.4  # InsightFace uses cosine distance, typically lower thresholds
        else:
    ...
    def generate_embedding(self, 
        ...
        try:
            ...
            elif method == EmbeddingMethod.FACE_RECOGNITION and self.face_recognition_available:
            ...
            embedding = self._ensure_dimension(embedding, self.embedding_config.embedding_dimension)
            ...
            self.embedding_stats["normalization_factors"].append(metadata.get("norm_factor", 1.0))
            ...
            embedding = np.random.randn(self.embedding_config.embedding_dimension).astype(np.float32)
            embedding = embedding / np.linalg.norm(embedding)
            metadata.update({
                "backend": "random_fallback",
                "error": str(e)
            })
            return embedding, metadata
    ...
    def _generate_face_recognition_embedding(self, face_image: np.ndarray) -> np.ndarray:
    ...
    def calculate_distance(self, embedding1: np.ndarray, embedding2: np.ndarray, 
        ...
        for embedding_file in embedding_files:
            try:
                ...
                unified_file = embedding_file.with_name(
                    embedding_file.name.replace("_embedding.npy", "_unified_embedding.npy")
                )
                metadata_file = embedding_file.with_name(
                    embedding_file.name.replace("_embedding.npy", "_embedding_metadata.json")
                )
                
                if unified_file.exists() and metadata_file.exists() and not force_regenerate:
    ...
    def _migrate_single_embedding(self, 
                                 original_embedding: np.ndarray, 
                                 target_method: EmbeddingMethod) -> Tuple[np.ndarray, Dict]:
        """Migrate a single embedding to unified format"""
        
        # Ensure correct dimensions
        embedding = self._ensure_dimension(original_embedding, self.embedding_config.embedding_dimension)
