...
class TrackingPerformanceAnalyzer:
    ...
    def generate_synthetic_detections(
        self, num_frames: int = 100, faces_per_frame: int = 3
    ) -> List[Tuple[List[FaceDetection], List[FaceRecognition]]]:
        ...
        for frame_idx in range(num_frames):
            ...
            if np.random.rand() > 0.9:  # 10% chance
                if len(face_positions) > 1 and np.random.rand() > 0.5:
                    ...
                    # Add a new face
                    new_face_id = (
                        max(face_positions.keys()) + 1 if face_positions else 0
                    )
    ...
    def run_tracker_comparison(
        self,
        test_data: List[Tuple[List[FaceDetection], List[FaceRecognition]]],
        tracker_name: str,
        tracker,
    ) -> Dict:
        ...
        for frame_idx, (detections, recognitions) in enumerate(test_data):
            ...
            self.metrics[tracker_name]["trajectory_count"].append(
                stats["active_trajectories"]
            )
            self.metrics[tracker_name]["stable_trajectories"].append(
                stats["stable_trajectories"]
            )
    ...
    def run_comparison(self, num_frames: int = 100, faces_per_frame: int = 3) -> Dict:
        ...
        logger.info(
            f"Starting tracking comparison with {num_frames} frames, {faces_per_frame} faces per frame"
        )
        ...
        custom_results = self.run_tracker_comparison(
            test_data, "custom", self.custom_tracker
        )

        # Test supervision tracker
        logger.info("Testing SupervisionFaceTracker...")
        supervision_results = self.run_tracker_comparison(
            test_data, "supervision", self.supervision_tracker
        )
    ...
    def _analyze_results(self, custom_results: Dict, supervision_results: Dict) -> Dict:
        ...
        comparison = {
            ...
            "tracking_quality": {
                ...
                "supervision_avg_trajectories": np.mean(
                    supervision_metrics["trajectory_count"]
                ),
                "custom_avg_stable": np.mean(custom_metrics["stable_trajectories"]),
                "supervision_avg_stable": np.mean(
                    supervision_metrics["stable_trajectories"]
                ),
                "stability_improvement": None,
                "consistency_improvement": None,
            },
            "memory_efficiency": {
                "custom_avg_memory": np.mean(custom_metrics["memory_usage"])
                if custom_metrics["memory_usage"]
                else 0,
                "supervision_avg_memory": np.mean(supervision_metrics["memory_usage"])
                if supervision_metrics["memory_usage"]
                else 0,
        ...
        if comparison["performance"]["custom_avg_time"] > 0:
            speed_ratio = (
                comparison["performance"]["custom_avg_time"]
                / comparison["performance"]["supervision_avg_time"]
            )
            comparison["performance"]["speed_improvement"] = (speed_ratio - 1) * 100
            comparison["performance"]["winner"] = (
                "supervision" if speed_ratio > 1 else "custom"
            )
        ...
        if custom_consistency > 0:
            comparison["tracking_quality"]["consistency_improvement"] = (
                (supervision_consistency - custom_consistency) / custom_consistency
            ) * 100

        if comparison["tracking_quality"]["custom_avg_stable"] > 0:
            stability_ratio = (
                comparison["tracking_quality"]["supervision_avg_stable"]
                / comparison["tracking_quality"]["custom_avg_stable"]
            )
            comparison["tracking_quality"]["stability_improvement"] = (
                stability_ratio - 1
            ) * 100

        if comparison["memory_efficiency"]["custom_avg_memory"] > 0:
            memory_ratio = (
                comparison["memory_efficiency"]["custom_avg_memory"]
                / comparison["memory_efficiency"]["supervision_avg_memory"]
            )
            comparison["memory_efficiency"]["memory_improvement"] = (
                memory_ratio - 1
            ) * 100
...
def main():
    ...
    try:
        ...
        for scenario_name, results in all_results.items():
            ...
            print(f"\n{scenario_name.upper()}:")
            print(
                f"  Test: {results['test_config']['num_frames']} frames, {results['test_config']['faces_per_frame']} faces/frame"
            )
            print(
                f"  Speed: {comp['performance']['speed_improvement']:+.1f}% (Supervision vs Custom)"
            )
            print(
                f"  Consistency: {comp['tracking_quality']['consistency_improvement']:+.1f}%"
            )
            print(
                f"  Stability: {comp['tracking_quality']['stability_improvement']:+.1f}%"
            )
