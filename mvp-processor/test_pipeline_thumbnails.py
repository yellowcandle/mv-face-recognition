...
def test_pipeline_thumbnails():
    ...
    try:
        ...
        print(
            f"📊 Video: {video_info['duration']:.1f}s, {video_info['width']}x{video_info['height']}, {video_info['fps']:.1f}fps"
        )
        ...
        if len(thumbnail_paths) == expected_thumbnails:
            print(
                f"✅ Correct thumbnail count: {len(thumbnail_paths)} (expected {expected_thumbnails})"
            )
        else:
            print(
                f"⚠️  Thumbnail count mismatch: got {len(thumbnail_paths)}, expected {expected_thumbnails}"
            )
        ...
        metadata = {
            ...
            "processing_info": {
                "thumbnail_interval": config.get("thumbnails", {}).get(
                    "interval", 60.0
                ),
        ...
        print(
            "✅ Thumbnail generation is properly integrated into the processing pipeline"
        )
