...
def test_embedding_path_manager():
    ...
    for contestant_id, nickname in test_contestants:
        ...
        # Find existing paths
        existing_emb, existing_meta = path_manager.find_existing_embedding_paths(contestant_id, nickname)
        if existing_emb:
            print(f"   ✅ Found embedding: {existing_emb.name}")
            print(f"   ✅ Found metadata: {existing_meta.name if existing_meta else 'None'}")
        else:
    ...
    if validation_results['missing_metadata']:
        print(f"   Files missing metadata: {validation_results['missing_metadata'][:3]}...")
    ...
    for i, emb_info in enumerate(all_embeddings[:5]):
        ...
        print(f"   {status} {emb_info['file_name']} ({unified}, {emb_info['naming_convention']})")
    
    if len(all_embeddings) > 5:
        print(f"   ... and {len(all_embeddings) - 5} more")
    
    # Show statistics
    id_based_count = sum(1 for e in all_embeddings if e['naming_convention'] == 'id_based')
    ...
    if photo_paths:
        ...
        print(f"   ❌ No photos found for contestant 1")
