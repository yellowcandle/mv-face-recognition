...
def test_enhanced_face_detection():
    ...
    print(
        f"✅ Loaded {len(face_detector.contestant_db.face_encodings)} contestant embeddings"
    )
    ...
    print(
        f"   📈 Embedding success rate: {(valid_embeddings / max(total_detections, 1) * 100):.1f}%"
    )

    if valid_embeddings > 0:
        print("   🎉 Enhanced face detection is working!")
    else:
        print("   ⚠️  No valid embeddings generated - may need further tuning")
