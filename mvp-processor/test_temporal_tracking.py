...
class TemporalTrackingValidator:
    ...
    def load_test_metadata(self) -> bool:
        ...
        metadata_path = Path(__file__).parent.parent / "metadata" / "test-video-mv2_metadata.json"
    ...
    def create_mock_detections(self, frame_test_data: FrameTestData) -> List[FaceDetection]:
    ...
    def create_mock_recognitions(self, frame_test_data: FrameTestData, detections: List[FaceDetection]) -> List[FaceRecognition]:
    ...
    def test_spatial_correlation_iou(self) -> Dict:
        ...
        test_cases = [
            ...
            ((100, 200, 200, 100), (100, 200, 200, 100), (0.95, 1.05)),  # Perfect overlap
            ((100, 200, 200, 100), (110, 210, 210, 110), (0.6, 0.8)),    # Significant overlap
        ...
        for i, (box1, box2, expected_range) in enumerate(test_cases):
            ...
            logger.info(f"IoU test {i+1}: {iou:.3f} (expected {min_expected:.1f}-{max_expected:.1f}) {'✓' if test_passed else '✗'}")
        
        results["success_rate"] = results["test_cases_passed"] / results["test_cases_total"]
        logger.info(f"IoU testing complete: {results['test_cases_passed']}/{results['test_cases_total']} passed")
    ...
    def test_trajectory_creation_and_management(self) -> Dict:
        ...
        for frame_idx, frame_test_data in enumerate(self.frame_data):
            ...
            # Update trajectories
            active_trajectories = self.face_tracker.update_trajectories(detections, recognitions)
            ...
            if frame_idx % 30 == 0:
                logger.info(f"Frame {frame_test_data.frame_number}: {current_active} active, {stable_count} stable trajectories")
        ...
        for trajectory in self.face_tracker.completed_trajectories:
            ...
            if trajectory.is_stable:
                trajectory_stats["stability_statistics"]["stable_trajectories_count"] += 1
            else:
                trajectory_stats["stability_statistics"]["unstable_trajectories_count"] += 1
        
        # Calculate averages
        if trajectory_stats["trajectory_durations"]:
            trajectory_stats["average_trajectory_duration"] = np.mean(trajectory_stats["trajectory_durations"])
            trajectory_stats["median_trajectory_duration"] = np.median(trajectory_stats["trajectory_durations"])
        else:
            trajectory_stats["average_trajectory_duration"] = 0.0
            trajectory_stats["median_trajectory_duration"] = 0.0
        
        logger.info(f"Trajectory management test complete:")
        logger.info(f"  Created: {trajectory_stats['trajectories_created']} trajectories")
        logger.info(f"  Expired: {trajectory_stats['trajectories_expired']} trajectories")
        logger.info(f"  Max active: {trajectory_stats['max_active_trajectories']} trajectories")
        logger.info(f"  Average duration: {trajectory_stats['average_trajectory_duration']:.2f}s")
    ...
    def test_confidence_aggregation(self) -> Dict:
        ...
        test_trajectory = FaceTrajectory(
            trajectory_id=999,
            first_frame=0,
            last_frame=0
        )
        ...
        timestamps = [i * 0.16 for i in range(len(confidence_sequence))]  # 6 FPS sampling
        ...
        aggregation_results = {
            ...
            "decay_factor": self.config.get("face_tracking", {}).get("confidence_smoothing", 0.95),
            "final_aggregated_confidence": 0.0,
            "stability_achieved": False
        }
        
        # Add detections with known confidences
        for i, (conf, timestamp, frame_num) in enumerate(zip(confidence_sequence, timestamps, frame_numbers)):
            ...
            test_trajectory.add_detection(detection, recognition, self.config.get("face_tracking", {}))
            
            aggregation_results["aggregated_confidences"].append(test_trajectory.aggregated_confidence)
            
            logger.info(f"Frame {frame_num}: confidence {conf:.3f} -> aggregated {test_trajectory.aggregated_confidence:.3f}")
        
        aggregation_results["final_aggregated_confidence"] = test_trajectory.aggregated_confidence
        ...
        logger.info(f"Confidence aggregation test complete:")
        logger.info(f"  Final aggregated confidence: {aggregation_results['final_aggregated_confidence']:.3f}")
        logger.info(f"  Stability achieved: {aggregation_results['stability_achieved']}")
    ...
    def test_tracking_statistics_generation(self) -> Dict:
        ...
        if stats.get("next_trajectory_id", 0) >= 1:
            checks.append({"check": "positive_trajectory_ids", "passed": True, "value": stats["next_trajectory_id"]})
        else:
            checks.append({"check": "positive_trajectory_ids", "passed": False, "value": stats.get("next_trajectory_id", 0)})
            validation_results["validation_passed"] = False
        ...
        if accounted_trajectories >= 0 and accounted_trajectories <= total_trajectories:
            checks.append({"check": "trajectory_accounting", "passed": True, 
                         "total": total_trajectories, "accounted": accounted_trajectories})
        else:
            checks.append({"check": "trajectory_accounting", "passed": False,
                         "total": total_trajectories, "accounted": accounted_trajectories})
            validation_results["validation_passed"] = False
        ...
        if 0 <= avg_duration <= 30.0:
            checks.append({"check": "reasonable_avg_duration", "passed": True, "value": avg_duration})
        else:
            checks.append({"check": "reasonable_avg_duration", "passed": False, "value": avg_duration})
            validation_results["validation_passed"] = False
        
        validation_results["reasonableness_checks"] = checks
        validation_results["raw_statistics"] = stats
        
        logger.info(f"Statistics validation: {'✓ PASSED' if validation_results['validation_passed'] else '✗ FAILED'}")
        for check in checks:
    ...
    def generate_comprehensive_report(self, test_results: Dict) -> Dict:
        ...
        report = {
            "test_summary": {
                ...
                "test_timestamp": self.metadata["processing_summary"]["processing_timestamp"],
                "frames_analyzed": len(self.frame_data),
                "total_detections": self.metadata["processing_summary"]["total_faces_detected"],
                "total_recognitions": self.metadata["processing_summary"]["total_recognitions"]
            },
            "configuration_tested": {
                "tracking_window": self.config.get("face_tracking", {}).get("tracking_window", "N/A"),
                "spatial_threshold": self.config.get("face_tracking", {}).get("spatial_threshold", "N/A"),
                "confidence_smoothing": self.config.get("face_tracking", {}).get("confidence_smoothing", "N/A"),
                "max_trajectory_gap": self.config.get("face_tracking", {}).get("max_trajectory_gap", "N/A")
            },
        ...
        criteria = {
            ...
            "confidence_aggregation_stability": {
                ...
                "actual": test_results["confidence_test"]["final_aggregated_confidence"],
                "passed": test_results["confidence_test"]["final_aggregated_confidence"] > 0
            },
        ...
        if test_results["iou_results"]["success_rate"] < 0.9:
            report["recommendations"].append("Consider tuning spatial_threshold for better IoU matching accuracy")
        
        if test_results["trajectory_stats"]["max_active_trajectories"] < 5:
            report["recommendations"].append("Test with more complex scenes to validate multi-face tracking")
            
        if test_results["trajectory_stats"]["average_trajectory_duration"] < 1.0:
            report["recommendations"].append("Consider increasing tracking_window for longer trajectory persistence")
        
        if not test_results["confidence_test"]["stability_achieved"]:
            report["recommendations"].append("Review confidence_threshold and min_stable_detections parameters")
        
        logger.info(f"Report generation complete. Overall acceptance: {'✓ PASSED' if overall_passed else '✗ FAILED'}")
    ...
    def run_full_test_suite(self) -> Dict:
        ...
        logger.info("="*40)
        test_results["trajectory_stats"] = self.test_trajectory_creation_and_management()
        ...
        logger.info("="*40)
        test_results["statistics_validation"] = self.test_tracking_statistics_generation()
...
def main():
    ...
    try:
        ...
        overall_passed = report.get("acceptance_criteria_validation", {}).get("overall_passed", False)
        ...
        print("\nAcceptance Criteria:")
        for criterion_name, criterion in report.get("acceptance_criteria_validation", {}).get("criteria", {}).items():
            status = "✅" if criterion["passed"] else "❌"
            print(f"  {status} {criterion_name}: {criterion['actual']} ({criterion['target']})")
