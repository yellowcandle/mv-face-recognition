...
from face_detector import FaceDetector, ContestantDatabase, FaceDetection
...
class FaceDetectionTester:
    ...
    def analyze_video_info(self, video_path: Path) -> Dict:
        ...
        try:
            ...
            logger.info(f"Video analysis: {info['resolution']}, {info['fps']:.1f}fps, {info['duration']:.1f}s")
            return info
    ...
    def sample_frames_for_testing(self, video_info: Dict, sample_count: int = 50) -> List[int]:
    ...
    def detect_faces_in_frame(self, frame: np.ndarray, frame_number: int, timestamp: float) -> DetectionStats:
        ...
        try:
            ...
            # Detect faces
            detections = self.face_detector.detect_faces(rgb_frame, timestamp, frame_number)
    ...
    def test_face_detection_accuracy(self, video_path: Path) -> TestResults:
        ...
        logger.info(f"Testing {len(test_frames)} frames from {video_info['frame_count']} total")
    ...
    def calculate_performance_metrics(self):
        ...
        self.results.overall_performance = {
            ...
            "frames_with_faces": sum(1 for s in self.results.detection_stats if s.faces_detected > 0),
            "face_detection_rate": sum(1 for s in self.results.detection_stats if s.faces_detected > 0) / len(self.results.detection_stats)
        }
        
        logger.info(f"Performance: {self.results.overall_performance['avg_fps']:.1f} FPS, "
    ...
    def calculate_quality_metrics(self):
        ...
        self.results.quality_metrics = {
            ...
            "total_faces_detected": sum(stat.faces_detected for stat in self.results.detection_stats),
            "meets_expectation_10_faces": self.results.overall_performance.get("avg_faces_per_frame", 0) >= 10
        }
    ...
    def generate_report(self, output_file: str = "face_detection_test_report.json"):
        ...
        def convert_numpy_types(obj):
            if isinstance(obj, (np.integer, np.int64, np.int32)):
                ...
                return convert_numpy_types(asdict(obj) if hasattr(obj, '__dataclass_fields__') else obj.__dict__)
            return obj
    ...
    def get_key_findings(self) -> List[str]:
        ...
        findings.append(f"Detection rate: {perf.get('face_detection_rate', 0):.1%} of frames had faces")  
        findings.append(f"Performance: {perf.get('avg_fps', 0):.1f} FPS average")
        findings.append(f"Accuracy: {perf.get('avg_faces_per_frame', 0):.1f} faces per frame")
        findings.append(f"Quality: {qual.get('avg_confidence', 0):.3f} average confidence")
