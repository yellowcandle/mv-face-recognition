...
def calculate_enhanced_face_similarity(bbox1, bbox2, embedding1=None, embedding2=None, embedding_weight=0.6):
    """
    ...
    if embedding1 is not None and embedding2 is not None:
        ...
        combined_similarity = (
            (1 - embedding_weight) * bbox_similarity + 
...
def update_enhanced_persistent_labels(matches, label_cache, current_frame, persistence_duration=300):
    """
    ...
    expired_keys = [
        key for key, data in label_cache.items() 
    ...
    for face, name in matches:
        ...
        if face_key in label_cache:
            ...
            trajectory_confidence = label_cache[face_key].get("trajectory_confidence", 0.5)
        ...
        label_cache[face_key] = {
            ...
            "first_seen": label_cache[face_key].get("first_seen", current_frame) if face_key in label_cache else current_frame,
            "confidence": getattr(face, "det_score", 1.0) if hasattr(face, "det_score") else 1.0,
            "trajectory_confidence": trajectory_confidence,
            "similarity_score": best_similarity,
            "detection_count": label_cache[face_key].get("detection_count", 0) + 1 if face_key in label_cache else 1,
        }
...
def get_trajectory_stability_metrics(label_cache, current_frame):
    ...
    for face_key, data in label_cache.items():
        ...
        stability_metrics[face_key] = {
            ...
            "is_stable": overall_stability > 0.7 and duration > 30  # Stable if good metrics and >1 second
        }
