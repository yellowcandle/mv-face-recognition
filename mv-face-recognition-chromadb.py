...
def parse_args():
    ...
    parser.add_argument("--specific-contestants", type=str, help="Comma-separated list of contestant names to include")
    parser.add_argument("--specific-videos", type=str, help="Comma-separated list of video filenames to process")
...
def is_quality_face(face, frame, min_size=30, blur_threshold=25):
    ...
    try:
        ...
        face_roi = frame[max(0, bbox[1]):min(frame.shape[0], bbox[3]), 
        ...
        for i, embedding in enumerate(embeddings):
            ...
            if isinstance(embedding, np.ndarray):
                ...
                if len(embedding) != self.embedding_dimension:
                    print(f"Warning: Expected embedding dimension {self.embedding_dimension}, got {len(embedding)}. Skipping...")
                    continue
    ...
    def match_face(self, face_embedding, n_results=1):
        ...
        if isinstance(face_embedding, np.ndarray):
            ...
            if len(face_embedding) != self.embedding_dimension:
                print(f"Warning: Query embedding dimension mismatch. Expected {self.embedding_dimension}, got {len(face_embedding)}")
                return None
        ...
        try:
            ...
            if results['distances'][0] and results['metadatas'][0]:
                ...
                if similarity > self.distance_threshold:
                    self.match_count += 1
                    return results['metadatas'][0][0]['name']  # First result, first metadata, name field
...
def process_frame(frame, known_embeddings, chroma_db=None, distance_threshold=0.18, debug=False, quality_check=False):
        ...
        for face in faces:
            ...
            if chroma_db is not None:
                ...
                if matched_name is None:
                    ...
                    matched_name = standard_match_face(face_embedding, known_embeddings, distance_threshold * 0.9)  # Lower threshold for fallback
            else:
            ...
            if matched_name:
                ...
                for name, embeddings_list in known_embeddings.items():
                    for known_embedding in embeddings_list:
                        ...
                        similarity = np.dot(face_embedding_flat, known_embedding_flat)
                        if isinstance(similarity, np.ndarray):
                            similarity = similarity.item() if similarity.size == 1 else similarity.mean()
                ...
                if best_name and debug:
                    print(f"Best match (below threshold): {best_name} with similarity {highest_similarity:.4f} (threshold: {distance_threshold})")
...
def recognize_faces_in_videos(videos_dir, selected_videos, known_embeddings, chroma_db=None, frame_skip=5, debug=False, quality_check=False):
    ...
    for video_name in bar:
        ...
        while cap.isOpened():
            ...
            if frame_count % frame_skip == 0:
                ...
                matches = process_frame(frame, known_embeddings, chroma_db, debug=debug, quality_check=quality_check)
                processed_count += 1
...
def main():
    ...
    for contestant in selected_contestants:
        ...
        if os.path.exists(embedding_file):
            ...
            if contestant_image is not None:
                embeddings = compute_embeddings([contestant_image], use_augmentation=args.augmentation)
                if embeddings:
    ...
    recognize_faces_in_videos(videos_dir, selected_videos, known_embeddings, chroma_db, args.frame_skip, debug=args.debug, quality_check=args.quality_check)
