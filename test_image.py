...
try:
    ...
    HAS_SKOPT = False
    console.print(
        "[yellow]Warning:[/yellow] scikit-optimize (skopt) not found. Bayesian optimization will not be available."
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test face recognition on an image with adjustable threshold."
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    result_path = os.path.join(
        result_image_dir,
        f"result_threshold_{RECOGNITION_THRESHOLD:.2f}_confidence_{CONFIDENCE_THRESHOLD:.2f}.jpeg",
    )
    ...
    if DEBUG:
        console.print(
            Panel(
                ...
                title="[yellow]Debug Status[/yellow]",
                expand=False,
            )
        )
    ...
    face_recognizer = FaceRecognizer(
        face_detector=face_detector, similarity_threshold=RECOGNITION_THRESHOLD, use_arcface=True
    )
    ...
    if DEBUG:
        console.print(
            Panel(
                ...
                title="[yellow]Parameters[/yellow]",
                expand=False,
            )
        )
        ...
        param_table.add_row(
            "Detection confidence threshold", str(face_detector.confidence_threshold)
        )
        param_table.add_row(
            "Recognition similarity threshold", str(face_recognizer.similarity_threshold)
        )
    ...
    if DEBUG:
        console.print(
            Panel(
                "[bold cyan]LOADING EMBEDDINGS FOR TARGET CONTESTANTS (DEBUGGING FOCUS)[/bold cyan]",
                title="[yellow]Embeddings[/yellow]",
                expand=False,
            )
        )
        ...
        if os.path.exists(embedding_path):
            try:
                ...
                if embedding.ndim == 2 and embedding.shape[0] == 1:
                    ...
                    console.print(
                        f"  [yellow]Warning:[/yellow] Embedding for {contestant_name} has unexpected shape {embedding.shape}. Skipping."
                    )
                    continue
                ...
                if DEBUG:
                    ...
                    console.print(
                        f"  [dim]Min/Max:[/dim] {np.min(embedding):.4f}/{np.max(embedding):.4f}"
                    )
            except Exception as e:
                console.print(
                    f"[red]Error loading embedding for {contestant_name} from {embedding_path}:[/red] {e}"
                )
        else:
            if DEBUG:
                console.print(
                    f"[yellow]Embedding file not found for target contestant:[/yellow] {contestant_name} at {embedding_path}"
                )
    ...
    if DEBUG:
        ...
        if missing_target_embeddings:
            console.print(
                f"[yellow]Warning:[/yellow] Missing embeddings for the following target contestants: {', '.join(missing_target_embeddings)}"
            )
        if not known_embeddings:
            console.print(
                "[yellow]Warning:[/yellow] No target embeddings were loaded. Recognition will not be possible against targets."
            )
    ...
    for person_id, embeddings_list in known_embeddings.items():
        if (
            embeddings_list and embeddings_list[0] is not None
        ):  # Ensure there's at least one valid embedding
    ...
    if DEBUG:
        console.print(
            Panel(
                ...
                title="[yellow]Detection Phase[/yellow]",
                expand=False,
            )
        )
    ...
    if DEBUG:
        ...
        for i, (face_embedding, result) in enumerate(zip(face_data, results, strict=False)):
    ...
    for i, result in enumerate(results, 1):
        ...
        text_width, text_height = draw.textbbox((0, 0), recognition_label, font=font)[
            2:
        ...
        draw.rectangle(
            [x1, recognition_box_y1, x1 + text_width + 10, recognition_box_y2], fill=box_color
        )
    ...
    console.print(
        Panel(
            "[bold cyan]RECOGNITION SUMMARY[/bold cyan]",
...
def evaluate_recognition(results, ground_truth_map, debug=False):
    ...
    if debug:
        console.print(
            Panel(
                "[bold cyan]AUTOMATED EVALUATION[/bold cyan]",
                title="[yellow]Evaluation[/yellow]",
                expand=False,
            )
        )
        eval_details_table = Table(
            title="Face Evaluation Details", show_header=True, header_style="bold magenta"
        )
    ...
    for i, result in enumerate(results):
        ...
        if debug:
            eval_details_table.add_row(
                str(face_idx_1_based), str(gt_person), str(recognized_person), score_str
            )
        ...
        if gt_person == "NOT_IN_DATASET":
            ...
            ):  # Should have been identified, but wasn't (below threshold)
...
def run_single_evaluation_iteration(
    current_recognition_threshold,
    current_detection_confidence,
    ...
    result_path = os.path.join(
        result_image_dir,
        f"result_eval_model_{recognition_model_name}_rec_{current_recognition_threshold:.2f}_det_{current_detection_confidence:.2f}.jpeg",
    )

    if DEBUG:
        console.print(
            Panel(
                f"[bold cyan]RUNNING FOR MODEL: {recognition_model_name}, REC_THRESH: {current_recognition_threshold:.2f}, DET_CONF: {current_detection_confidence:.2f}[/bold cyan]",
                title="[yellow]Evaluation Iteration[/yellow]",
                expand=False,
            )
        )
    ...
    embeddings_base_dir = os.path.join(
        project_root_path, "source", "photo", "contestants", "embeddings"
    )
    ...
    for contestant_name in target_contestants_for_debug:
        ...
        if os.path.exists(embedding_path):
            try:
                ...
                if DEBUG:
                    console.print(f"[red]Error loading {contestant_name}:[/red] {e}")

    if DEBUG:
        console.print(
            f"[cyan]Loaded {loaded_count}/{len(target_contestants_for_debug)} target embeddings for this run.[/cyan]"
        )
    ...
    if DEBUG and SHOW_ALL_SIMILARITIES:
        ...
        for i, (face_embedding, result) in enumerate(zip(face_data, results, strict=False)):
            console.print(
                f"  [magenta]Face {i + 1}[/magenta] (Recognized: {result['person_id'] if result['person_id'] else 'None'}, Score: {result['confidence']:.4f}):"
            )
            similarity_table = Table(show_header=False, box=None)
            similarity_table.add_column("Contestant", style="cyan")
            similarity_table.add_column("Score", style="yellow")
    ...
    try:
        ...
        if DEBUG:
            console.print("[yellow]Warning:[/yellow] Custom font not found. Using default font.")
    ...
    if DEBUG:
        console.print(
            f"[green]Saved result image for Model: {recognition_model_name}, Rec: {current_recognition_threshold:.2f}, Det: {current_detection_confidence:.2f} to {result_path}[/green]"
        )
...
def objective_function(recognition_threshold, detection_confidence, recognition_model):
    ...
    if PROJECT_ROOT is None or FONT_PATH is None or GROUND_TRUTH_MAP is None or ARGS is None:
        console.print(
            "[red]Error:[/red] Global paths/config not initialized for objective function."
        )
    ...
    if not eval_summary:
        if ARGS.debug:
            console.print(
                "[yellow]Evaluation failed for this iteration, returning low score.[/yellow]"
            )
    ...
    if ARGS.debug:
        console.print(
            f"[dim]Params: RecModel={recognition_model}, RecThresh={recognition_threshold:.2f}, DetConf={detection_confidence:.2f} -> Score: {score:.2f}[/dim]"
        )
...
def current_objective_function_for_skopt(
    recognition_threshold, detection_confidence, recognition_model
):
...
def run_bayesian_optimization(n_calls=30, n_random_starts=10):
    ...
    if not HAS_SKOPT:
        console.print(
            "[yellow]Skipping Bayesian Optimization: scikit-optimize is not installed.[/yellow]"
        )
        return None

    console.print(
        Panel(
            f"[bold cyan]Starting Bayesian optimization with {n_calls} total evaluations ({n_random_starts} random)...[/bold cyan]",
            title="[yellow]Bayesian Optimization[/yellow]",
            expand=False,
        )
    )

    # DIMENSIONS is now defined globally, so we use it directly.
    console.print("[dim]Optimizing parameters based on globally defined DIMENSIONS:[/dim]")
    console.print(
        f"[dim]- Recognition threshold: {DIMENSIONS[0].bounds if DIMENSIONS and len(DIMENSIONS) > 0 else 'N/A'}[/dim]"
    )
    console.print(
        f"[dim]- Detection confidence: {DIMENSIONS[1].bounds if DIMENSIONS and len(DIMENSIONS) > 1 else 'N/A'}[/dim]"
    )
    console.print(
        f"[dim]- Recognition models: {DIMENSIONS[2].categories if DIMENSIONS and len(DIMENSIONS) > 2 else 'N/A'}[/dim]"
    )
    ...
    opt_result_path = os.path.join(
        PROJECT_ROOT, "source", "images", "test", "bayesian_optimization_results.pkl"
    )
...
def analyze_optimization_results(result, project_root_path):
    ...
    console.print(
        Panel(
            "[bold cyan]BAYESIAN OPTIMIZATION RESULTS[/bold cyan]",
            title="[yellow]Optimization Analysis[/yellow]",
            expand=False,
        )
    )
    ...
    console.print("[bold]Best parameters found:[/bold]")
    ...
    try:
        ...
        conv_plot_path = os.path.join(
            project_root_path, "source", "images", "test", "optimization_convergence.png"
        )
        ...
        console.print(
            "[dim]Objective plots can be generated manually using skopt.plots.plot_objective(result).[/dim]"
        )
    ...
    trials_csv_path = os.path.join(
        project_root_path, "source", "images", "test", "optimization_trials_summary.csv"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Start recognition similarity threshold for grid search.",
    )
    parser.add_argument(
        "--end_threshold",
        type=float,
        default=0.8,
        help="End recognition similarity threshold for grid search.",
    )
    parser.add_argument(
        "--step_threshold",
        type=float,
        default=0.05,
        help="Step for recognition similarity threshold for grid search.",
    )
    parser.add_argument(
        "--start_confidence",
        type=float,
        default=0.3,
        help="Start detection confidence for grid search.",
    )
    parser.add_argument(
        "--end_confidence",
        type=float,
        default=0.7,
        help="End detection confidence for grid search.",
    )
    parser.add_argument(
        "--step_confidence",
        type=float,
        default=0.1,
        help="Step for detection confidence for grid search.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of calls (evaluations) for Bayesian optimization.",
    )
    parser.add_argument(
        "--n_random_starts",
        type=int,
    ...
    # General arguments
    parser.add_argument(
        "--fixed_recognition_model",
        type=str,
        default="sface",
        choices=["sface", "arcface", "opencv_dnn"],
        help="Fixed recognition model if not optimizing it. Note: 'opencv_dnn' is excluded from --optimize runs due to errors.",
    )
    parser.add_argument(
        "--fixed_recognition_threshold",
        type=float,
        help="Fixed recognition threshold if not optimizing/gridding.",
    )
    parser.add_argument(
        "--fixed_detection_confidence",
        type=float,
        help="Fixed detection confidence if not optimizing/gridding.",
    )
    ...
    ground_truth_map_input = [
        ...
        {"Face 7": "Mei Mei"},
        {"Face 8": "阿妹"},
        {"Face 9": "NOT_IN_DATASET"},
        {"Face 10": "NOT_IN_DATASET"},
    ]
    ...
    if ARGS.optimize:
        if not HAS_SKOPT:
            console.print(
                "[red]Cannot run optimization because scikit-optimize is not installed. Exiting.[/red]"
            )
            exit(1)
        ...
        if opt_result:
            ...
            console.print(
                Panel(
                    "[bold cyan]FINAL TEST WITH BEST OPTIMIZED PARAMETERS[/bold cyan]",
                    title="[yellow]Final Test[/yellow]",
                    expand=False,
                )
            )
            ...
            if final_eval_summary:
                ...
                final_summary_table.add_row(
                    "recognition_threshold (optimized)", f"{best_params_from_opt[0]:.3f}"
                )
                final_summary_table.add_row(
                    "detection_confidence (optimized)", f"{best_params_from_opt[1]:.3f}"
                )
                final_summary_table.add_row(
                    "recognition_model (optimized)", str(best_params_from_opt[2])
                )
                ...
                best_summary_path = os.path.join(
                    PROJECT_ROOT, "source", "images", "test", "best_optimized_run_summary.csv"
                )
                best_summary_df.to_csv(best_summary_path, index=False)
                console.print(
                    f"[green]Summary for best optimized run saved to {best_summary_path}[/green]"
                )

    else:  # Fallback to grid search or single run if --optimize is not used
        console.print(
            Panel(
                "[bold cyan]Running Grid Search / Fixed Parameter Evaluation[/bold cyan]",
                title="[yellow]Grid Search/Fixed Params[/yellow]",
                expand=False,
            )
        )
        recognition_models_to_test = [
            ARGS.fixed_recognition_model
        ]  # For now, grid search only on one model
        ...
        if ARGS.fixed_recognition_threshold is not None:
            ...
            recognition_thresholds_to_test = np.arange(
                ARGS.start_threshold, ARGS.end_threshold + ARGS.step_threshold, ARGS.step_threshold
            )

        if ARGS.fixed_detection_confidence is not None:
            detection_confidences_to_test = [ARGS.fixed_detection_confidence]
        else:
            detection_confidences_to_test = np.arange(
                ARGS.start_confidence,
                ARGS.end_confidence + ARGS.step_confidence,
                ARGS.step_confidence,
            )
        ...
        for model_name in recognition_models_to_test:
            console.print(
                Panel(
                    f"[bold cyan]Evaluating Model: {model_name}[/bold cyan]",
                    title="[yellow]Model Evaluation[/yellow]",
                    expand=False,
                )
            )
        ...
        console.print(
            Panel(
                "[bold cyan]OVERALL GRID EVALUATION RESULTS[/bold cyan]",
                title="[yellow]Grid Results[/yellow]",
                expand=False,
            )
        )
        if all_eval_summaries:
            ...
            cols_order = [
                ...
                "incorrect_recognitions (misidentified_known)",
                "missed_known_faces (FN)",
                "false_positives_on_unknown",
                "correctly_unidentified_unknowns (TN_unknown)",
                "total_detected_faces",
                "total_gt_known_faces_in_image",
                "total_gt_unknown_faces_in_image",
            ]
            ...
            summary_csv_path = os.path.join(
                ...
                "images",
                "test",
                "evaluation_summary_grid_model_conf_thresh.csv",
            )
            ...
            if best_params_grid:
                ...
                console.print(
                    f"[dim]source/images/test/result_eval_model_{best_params_grid['recognition_model']}_rec_{best_params_grid['recognition_threshold']:.2f}_det_{best_params_grid['detection_confidence']:.2f}.jpeg[/dim]"
                )
        else:
            console.print(
                "[yellow]No evaluation summaries were generated from grid search.[/yellow]"
            )
