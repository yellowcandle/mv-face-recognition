from src.config.config import Config, DetectionConfig, SegmentationConfig, RecognitionConfig
...
def main():
    ...
    config = Config(
        ...
        recognition=RecognitionConfig(
            model_path=os.path.join(project_root, "models", "face_recognition_mobilenet.caffemodel")
        )
    )
    ...
    face_recognizer = FaceRecognizer(
        recognition_model_path=os.path.join(project_root, "models", "face_recognition_mobilenet.caffemodel"),
    ...
    for face_embedding, bbox in faces:
        ...
        print(f"Detected {label} at coordinates ({int(x1)}, {int(y1)}, {int(x2)}, {int(y2)})")
