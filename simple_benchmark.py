...
def parse_args():
    ...
    parser.add_argument("--contestants", type=int, default=5, help="Number of contestants to include")
...
def load_contestants_data(num_contestants=None):
    ...
    if num_contestants is not None and num_contestants > 0 and num_contestants < len(all_contestants):
...
def load_embeddings(contestants_dir, contestant_info, selected_contestants, std_recognizer, detector):
...
def load_chromadb_embeddings(contestants_dir, contestant_info, selected_contestants, std_recognizer, detector, chroma_recognizer):
...
def benchmark_matching(video_path, std_embeddings, chroma_embeddings, std_recognizer, detector, iterations=1, frame_limit=30, verbose=False):
    ...
    for iteration in range(iterations):
        ...
        with tqdm(total=frame_limit, desc="Processing frames") as pbar:
            while frame_count < frame_limit:
                ...
                if can_use_chromadb:
                    start_time = time.time()
                    chroma_results = chroma_embeddings.identify_faces(frame, detector, std_recognizer)
                ...
                if verbose and frame_count % 10 == 0:
                    print(f"Frame {frame_count}: Standard: {std_time:.4f}s, ChromaDB: {chroma_time:.4f}s" if can_use_chromadb else
                          f"Frame {frame_count}: Standard: {std_time:.4f}s")
...
def benchmark_with_frames(frame_paths, std_embeddings, chroma_embeddings, std_recognizer, detector, iterations=1, verbose=False):
        ...
        with tqdm(total=len(frame_paths), desc="Processing frames") as pbar:
            for i, frame_path in enumerate(frame_paths):
                ...
                if verbose and (i+1) % 5 == 0:
                    print(f"Frame {i+1}: Standard: {std_time:.4f}s, ChromaDB: {chroma_time:.4f}s" if can_use_chromadb else
                          f"Frame {i+1}: Standard: {std_time:.4f}s")
...
def analyze_results(benchmark_results):
    ...
    std_times = np.array([item for sublist in benchmark_results["standard"]["times"] for item in sublist])
    ...
    if benchmark_results["chromadb"]:
        chroma_times = np.array([item for sublist in benchmark_results["chromadb"]["times"] for item in sublist])
