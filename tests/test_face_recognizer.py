...
import numpy as np
import pytest
from conftest import TEST_DATA_DIR, load_image  # Helpers from conftest.py
...
def get_a_face_image(image_path, face_detector):
    ...
    assert face_crop is not None and face_crop.size > 0, (
        "Failed to extract face for recognizer test setup."
    )
...
def test_add_face_to_database(
    ...
    _, full_image = get_a_face_image(
        sample_image_known_person, face_detector
    )  # We need full image for add_face
    ...
    assert person_id in face_recognizer.face_database, (
        f"{person_id} not found in database after adding."
    )
    assert isinstance(face_recognizer.face_database[person_id], np.ndarray), (
        "Stored embedding should be a numpy array."
    )
...
def test_identify_unknown_face(
    recognizer_fixture_name,
    sample_image_known_person,
    sample_image_multiple_faces,
    face_detector_insightface,
    face_detector_opencv,
    request,
):
    ...
    for result in results:  # Check all detected faces in the unknown image
        if result["person_id"] is not None:
            ...
            print(
                f"Warning: An 'unknown' face was identified as {result['person_id']} with confidence {result['confidence']}. This might be due to dummy test data similarity or a loose threshold."
            )
...
def test_identify_face_no_face_in_image(recognizer_fixture_name, sample_image_no_face, request):
    ...
    assert results == [], (
        "identify_face should return an empty list if no faces are detected in the input image."
    )


def test_get_embedding_method(
    face_recognizer_arcface, sample_image_known_person, face_detector_insightface
):
    ...
    assert loaded_embedding is None, (
        f"get_embedding for non-existent nickname '{non_existent_nickname}' should return None."
    )
