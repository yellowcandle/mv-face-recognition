import pytest
import numpy as np
from src.recognition.face_recognizer import FaceRecognizer # For _compute_similarity
...
def test_compute_similarity_identical_embeddings(face_recognizer_arcface):
    ...
    assert np.isclose(similarity, 1.0), f"Similarity for identical embeddings should be 1.0, got {similarity}."
...
def test_compute_similarity_orthogonal_embeddings(face_recognizer_arcface):
    ...
    similarity = recognizer._compute_similarity(emb1, emb2)
    assert np.isclose(similarity, 0.0), f"Similarity for orthogonal embeddings should be 0.0, got {similarity}."
...
def test_compute_similarity_opposite_embeddings(face_recognizer_arcface):
    ...
    similarity = recognizer._compute_similarity(emb1, emb2)
    assert np.isclose(similarity, -1.0), f"Similarity for opposite embeddings should be -1.0, got {similarity}."
...
def test_compute_similarity_zero_norm_embeddings(face_recognizer_arcface):
    ...
    assert np.isclose(similarity1, 0.0), f"Similarity with first zero embedding should be 0.0, got {similarity1}."
    
    # Case 2: Second embedding is zero
    similarity2 = recognizer._compute_similarity(emb_valid, emb_zero)
    assert np.isclose(similarity2, 0.0), f"Similarity with second zero embedding should be 0.0, got {similarity2}."
    
    # Case 3: Both embeddings are zero
    similarity3 = recognizer._compute_similarity(emb_zero, emb_zero)
    assert np.isclose(similarity3, 0.0), f"Similarity with both zero embeddings should be 0.0, got {similarity3}."
...
def test_identification_with_varying_similarity_scores(recognizer_fixture_name, sample_image_known_person, face_detector_insightface, face_detector_opencv, request):
    """
    ...
    for case_name, target_score in scores_to_test.items():
        ...
        if case_name == "above_threshold":
            ...
            assert identified_person is None, f"Failed for {case_name} (score at threshold), expected None ID due to strict '>' comparison."
            assert np.isclose(confidence, target_score)
...
@pytest.mark.parametrize("recognizer_fixture_name", ["face_recognizer_arcface", "face_recognizer_opencv_dnn"])
def test_identify_face_empty_database(recognizer_fixture_name, sample_image_known_person, face_detector_insightface, face_detector_opencv, request):
    ...
    if not results: # No face detected in sample_image_known_person by this detector
        pytest.skip(f"No face detected in {sample_image_known_person} by {face_detector.backend}, skipping empty DB content check.")
    ...
    for result in results:
        ...
        assert result["confidence"] == -1, "Confidence should be -1 (or initial best_similarity) for empty database."
