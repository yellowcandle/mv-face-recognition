...
def get_face_image_from_sample(image_path, face_detector):
    ...
    assert face_crop is not None and face_crop.size > 0, f"Failed to extract face from {image_path}."
    return face_crop

@pytest.mark.parametrize("recognizer_fixture_name", ["face_recognizer_arcface", "face_recognizer_opencv_dnn"])
def test_embedding_dimensionality(recognizer_fixture_name, sample_image_known_person, face_detector_insightface, face_detector_opencv, request):
    ...
    expected_dim = EXPECTED_EMBEDDING_DIM_SFACE if "arcface" in recognizer_fixture_name else EXPECTED_EMBEDDING_DIM_CV_DNN
...
@pytest.mark.parametrize("recognizer_fixture_name", ["face_recognizer_arcface", "face_recognizer_opencv_dnn"])
def test_embedding_normalization(recognizer_fixture_name, sample_image_known_person, face_detector_insightface, face_detector_opencv, request):
...
@pytest.mark.parametrize("recognizer_fixture_name", ["face_recognizer_arcface", "face_recognizer_opencv_dnn"])
def test_embedding_consistency_same_image(recognizer_fixture_name, sample_image_known_person, face_detector_insightface, face_detector_opencv, request):
...
@pytest.mark.parametrize("recognizer_fixture_name", ["face_recognizer_arcface"]) # OpenCV DNN might be less robust
def test_embedding_robustness_minor_variations(recognizer_fixture_name, sample_image_known_person, face_detector_insightface, request):
    """
    ...
    for variation_name, func in variations.items():
        ...
        assert embedding_varied is not None, f"Embedding generation failed for variation '{variation_name}'."
...
def test_embedding_on_problematic_input(face_recognizer_arcface):
    ...
    assert embedding_zeros.shape[0] == EXPECTED_EMBEDDING_DIM_SFACE, f"Fallback embedding for zero image has wrong dim for SFace, expected {EXPECTED_EMBEDDING_DIM_SFACE}."
    assert np.linalg.norm(embedding_zeros) > 1e-5, "Embedding for zero image should not be a zero vector (should be random unit vector as per implementation)."
    assert np.isclose(np.linalg.norm(embedding_zeros), 1.0, atol=1e-5), "Embedding for zero image should be normalized."
    ...
    assert embedding_random.shape[0] == EXPECTED_EMBEDDING_DIM_SFACE, f"Fallback embedding for random image has wrong dim for SFace, expected {EXPECTED_EMBEDDING_DIM_SFACE}."
    assert np.isclose(np.linalg.norm(embedding_random), 1.0, atol=1e-5), "Embedding for random image should be normalized."
