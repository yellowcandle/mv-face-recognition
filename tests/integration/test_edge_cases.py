...
def mock_face_detector():
    ...
    detector.detect_faces.side_effect = NotImplementedError(
        "Detection for edge cases missing"
    )
    return detector
...
def mock_face_matcher():
    ...
    matcher.match_face.side_effect = NotImplementedError(
        "Matching for edge cases missing"
    )
    return matcher
...
def test_low_quality_video_contract(
    mock_face_detector, mock_video_processor, temp_low_quality_image
):
    ...
    mock_face_detector.detect_faces.side_effect = NotImplementedError(
        "Low quality detection missing"
    )
...
def test_many_faces_frame_contract(mock_face_detector, mock_face_matcher):
    ...
    mock_face_detector.extract_embeddings.side_effect = (
        lambda img, bbox: np.random.rand(512)
    )
    ...
    with pytest.raises(NotImplementedError):
        mock_face_detector.detect_faces(
...
def test_similar_faces_contract(mock_face_matcher):
    ...
    mock_face_matcher.match_face.side_effect = NotImplementedError(
        "Similarity handling missing"
    )

    with pytest.raises(NotImplementedError):
        mock_face_matcher.match_face(
            embedding=np.random.rand(512), threshold=0.6
        )
        mock_face_matcher.match_face(
            embedding=np.random.rand(512), threshold=0.6
        )
...
def test_memory_limit_exceeded_contract(mock_video_processor):
    ...
    mock_video_processor.process_video.side_effect = MemoryError(
...
def test_edge_case_combinations_contract(mock_video_processor):
    ...
    mock_video_processor.process_video.side_effect = NotImplementedError(
        "Mixed edge cases handling missing"
    )
