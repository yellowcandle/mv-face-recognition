...
from src.cli.main import status_command, list_command
...
def test_status_command_contract(mock_embedding_manager):
    ...
    with patch('src.services.embedding_manager.EmbeddingManager', return_value=mock_embedding_manager):
...
def test_system_status_json_contract(mock_embedding_manager):
    ...
    expected_status = {
        ...
        "contestants_count": 2,
        "embeddings_count": 5,
        "model_info": {"name": "buffalo_l", "version": "insightface-0.7.3"},
        "hardware": {"gpu_available": False, "gpu_type": "CPU"},
        "storage": {"used_gb": 0.1, "available_gb": 100.0},
...
def test_list_table_format_contract(mock_embedding_manager):
    ...
    expected_table = """ID   Name        Nickname    Age  Photos  Embeddings
1    蘇雅琳      Ivy So      20   1       1
2    黃雅慧      咖喱        27   1       1
"""
...
def test_results_json_review_contract(mock_process_result):
    ...
    with pytest.raises(ImportError):
        formatted_results = review_results(mock_process_result)
    ...
    expected_json = {
        ...
        "unique_contestants": [1, 2, 5, 12, 23, 45],
        "processing_stats": {
...
def test_contestant_search_contract(mock_embedding_manager):
    ...
    with pytest.raises(ImportError):
        results = search_contestants("蘇雅琳", manager=mock_embedding_manager)
...
def test_results_statistics_contract(mock_process_result):
    ...
    with pytest.raises(ImportError):
        stats = calculate_statistics(mock_process_result)
...
def test_status_health_check_contract(mock_embedding_manager):
    ...
    with pytest.raises(ImportError):
        health = get_system_health(mock_embedding_manager)
...
def test_storage_status_contract(mock_embedding_manager):
    ...
    with pytest.raises(ImportError):
        storage_status = get_storage_status(mock_embedding_manager)
...
def test_last_update_validation_contract(mock_embedding_manager):
    ...
    with pytest.raises(ImportError):
        is_recent = validate_last_update(mock_embedding_manager.last_update())
...
def test_contestant_distribution_contract(mock_embedding_manager, mock_process_result):
    ...
    with pytest.raises(ImportError):
        distribution = analyze_contestant_distribution(
