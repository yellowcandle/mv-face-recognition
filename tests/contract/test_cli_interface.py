...
def test_init_success():
    ...
    result = run_cli(["init", "--contestant-csv", TEST_CSV, "--photos-dir", "test_photos", "--force"])
    assert result.returncode == 0
...
def test_process_missing_input():
    ...
    assert "error: the following arguments are required: input_file" in result.stderr.lower()
...
def test_process_success():
    ...
    result = run_cli(["process", TEST_VIDEO, "--output-dir", "test_output", "--mode", "frames", "--confidence", "0.3"])
    assert result.returncode == 0
