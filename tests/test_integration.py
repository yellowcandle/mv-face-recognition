...
def integrated_detector_recognizer():
    ...
    try:
        ...
        recognizer = FaceRecognizer(face_detector=detector, similarity_threshold=0.7, use_arcface=True) 
...
def test_full_pipeline_add_and_identify_known_person(integrated_detector_recognizer, sample_image_known_person):
    """
...
def test_full_pipeline_identify_unknown_person(integrated_detector_recognizer, sample_image_known_person, sample_image_multiple_faces):
    """
    ...
    try:
        ...
        pytest.fail(f"Failed to add known person from {sample_image_known_person} for unknown person test.")
    ...
    if not identification_results_unknown:
        ...
        pytest.skip(f"No faces detected in {sample_image_multiple_faces} by InsightFace detector, cannot test unknown identification logic fully.")
    ...
    for result in identification_results_unknown:
        if result["person_id"] == known_person_id:
            ...
            print(f"Warning: A face in '{sample_image_multiple_faces}' was identified as known person '{known_person_id}' "
...
def test_pipeline_with_no_face_in_image_for_identification(integrated_detector_recognizer, sample_image_no_face):
    ...
    assert results == [], "identify_face should return an empty list if no faces are detected in the input image for identification."
