...
@pytest.mark.parametrize("detector_fixture_name", ["face_detector_opencv", "face_detector_insightface"])
def test_detect_single_face(detector_fixture_name, sample_image_known_person, request):
    ...
    assert len(detections) > 0, f"Expected at least one face to be detected in {sample_image_known_person} with {face_detector.backend}."
    
    # Check properties of the first detection
    first_detection = detections[0]
    if face_detector.backend == FaceDetector.BACKEND_INSIGHTFACE:
        assert isinstance(first_detection, InsightFaceObject), "InsightFace backend should return InsightFaceObject."
        assert hasattr(first_detection, 'bbox'), "InsightFaceObject should have 'bbox' attribute."
        assert hasattr(first_detection, 'kps'), "InsightFaceObject should have 'kps' attribute (keypoints)."
        assert hasattr(first_detection, 'det_score'), "InsightFaceObject should have 'det_score' attribute."
...
def test_no_face_detection(detector_fixture_name, sample_image_no_face, request):
    ...
    assert len(detections) == 0, f"Expected no faces to be detected in {sample_image_no_face} with {face_detector.backend}, but found {len(detections)}."
...
def test_multiple_face_detection(detector_fixture_name, sample_image_multiple_faces, request):
    ...
    assert len(detections) > 0, f"Expected faces to be detected in {sample_image_multiple_faces} with {face_detector.backend}."
    if len(detections) < 2:
        print(f"Warning: {face_detector.backend} detected {len(detections)} face(s) in multiple_faces image. Expected >=2 for a robust detector.")
...
def test_face_extraction(face_detector_insightface, sample_image_known_person):
    ...
    assert face_crop.shape[0] > 0 and face_crop.shape[1] > 0, "Face crop dimensions should be positive."
    ...
    assert extracted_invalid is None, "extract_face should return None for completely out-of-bounds bbox."
    ...
    assert extracted_malformed is None, "extract_face should return None for malformed bbox (x2<x1 or y2<y1)."
