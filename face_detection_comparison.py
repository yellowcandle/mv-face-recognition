...
class FaceDetectionComparison:
    ...
    def compare_on_image(self, image):
        ...
        # Add labels
        cv2.putText(combined, f"MediaPipe ({len(mp_faces)} faces, {mp_time:.3f}s)", 
                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(combined, f"InsightFace ({len(if_faces)} faces, {if_time:.3f}s)", 
                   (image.shape[1] + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
def main():
    ...
    while current_frame < total_frames:
        ...
        print(f"MediaPipe: {results['mediapipe']['faces']} faces in {results['mediapipe']['time']:.3f}s")
        print(f"InsightFace: {results['insightface']['faces']} faces in {results['insightface']['time']:.3f}s")
    ...
    if frame_count > 0:
        ...
        print(f"MediaPipe - Avg time: {total_mp_time/frame_count:.3f}s, Total faces: {total_mp_faces}, Avg faces per frame: {total_mp_faces/frame_count:.1f}")
        print(f"InsightFace - Avg time: {total_if_time/frame_count:.3f}s, Total faces: {total_if_faces}, Avg faces per frame: {total_if_faces/frame_count:.1f}")
