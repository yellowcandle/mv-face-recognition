    ...
    try:
        ...
        for i in range(blocks_per_side):
            for j in range(blocks_per_side):
                ...
                hist, _ = np.histogram(block_ang.flatten(), bins=9, 
...
def generate_embeddings_for_contestant(contestant_id: str, contestants_info: dict):
    ...
    if saved > 0:
        logger.info(f"✅ Generated embedding for {nickname} from {len(all_features)} photos")
        return True
    else:
...
def regenerate_all_embeddings(force: bool = False):
    ...
    print(f"\n{'='*50}")
    print(f"EMBEDDING REGENERATION SUMMARY")
    print(f"{'='*50}")
...
def main():
    ...
    parser.add_argument("--contestant-id", type=str, help="Regenerate specific contestant")
    
    args = parser.parse_args()
    
    if args.contestant_id:
        contestants_info = load_contestant_info()
        success = generate_embeddings_for_contestant(args.contestant_id, contestants_info)
        if success:
