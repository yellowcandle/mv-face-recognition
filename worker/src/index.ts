 ...
 * This function actually calls the Modal webhook to start processing
 */
async function triggerModalProcessing(videoUrl: string, videoName: string, env: Env): Promise<{success: boolean; jobId?: string; error?: string}> {
  try {
    if (!env.MODAL_TOKEN) {
      return { success: false, error: 'MODAL_TOKEN not configured' };
    }

    logger.info('Calling Modal webhook', { videoUrl, videoName });

    // Call Modal webhook endpoint
    // Note: You'll need to deploy the Modal app first and get the actual URL
    const modalWebhookUrl = 'https://yellowcandle--mv-face-recognition-main-webhook-dev.modal.run/webhook_process_video';
    ...
    if (!response.ok) {
      ...
      logger.error('Modal webhook error', { status: response.status, error: errorText });
      return { success: false, error: `Modal API error: ${response.status} - ${errorText}` };
    }

    const result = await response.json() as { job_id: string };
    logger.info('Modal processing started', { jobId: result.job_id });

    return { success: true, jobId: result.job_id };

  } catch (error) {
    const err = error as Error;
    logger.error('Failed to trigger Modal webhook', { error: err.message });
...
async function handleTriggerModal(request: Request, env: Env, corsHeaders: CorsHeaders): Promise<Response> {
  try {
    ...
    // Actually trigger Modal processing
    const modalResult = await triggerModalProcessing(video_url, video_name || 'unknown_video', env);

    if (!modalResult.success) {
      logger.error('Failed to trigger Modal', { error: modalResult.error });
      return new Response(JSON.stringify({
        success: true,
        job_id: jobId,
        warning: 'Job queued but failed to trigger Modal automatically',
        error: modalResult.error,
        message: 'You can manually trigger with:',
        command: 'modal run scripts/modal_app.py --process-videos',
        video_url,
        video_name,
        status: 'queued_manual_required'
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    jobData.modal_job_id = modalResult.jobId;
    jobData.status = 'processing';
    await env.METADATA_KV.put(`modal_job_${jobId}`, JSON.stringify(jobData), {
      expirationTtl: 86400 * 7
    });

    logger.info('Modal processing started', { jobId, modalJobId: modalResult.jobId });

    return new Response(JSON.stringify({
      success: true,
      job_id: jobId,
      modal_job_id: modalResult.jobId,
      message: 'Modal processing started successfully',
      video_url,
      video_name,
      status: 'processing'
    }), {
