        ...
        for _, row in display_df.iterrows():
            ...
            for col in columns:
                if col == 'confidence' and col in row:
                    ...
                    values.append(f"{row[col]:.2%}" if isinstance(row[col], float) else str(row[col]))
                elif col in row:
...
class StatusDisplay:
    ...
    def update(self, 
        ...
        if self.stats["faces_detected"] > 0:
            self.stats["recognition_rate"] = self.stats["faces_recognized"] / self.stats["faces_detected"]
    ...
    def _update_display(self, layout):
        ...
        stats_content = [
            Text(f"📊 Processing Statistics:", style="bold yellow"),
    ...
    def _print_status(self):
        ...
        print(f"\rProcessed: {self.stats['frames_processed']} frames | "
              f"Detected: {self.stats['faces_detected']} faces | "
              f"Recognized: {self.stats['faces_recognized']} | "
              f"FPS: {self.stats['current_frame_rate']:.1f}", end="")
