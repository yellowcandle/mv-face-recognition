...
from src.utils.performance import image_cache, optimize_test_images
from src.utils.image_utils import (
    load_image, save_image, is_test_image, 
    enhance_test_image, create_composite_image,
    get_all_test_images
)
...
class TestImageOptimizer:
    ...
    def preprocess_all_test_images(self) -> int:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [executor.submit(self._preprocess_single_image, img_path) for img_path in test_files]
        ...
        print(f"Preprocessed {self.processed_images} test images in {self.processing_time:.2f} seconds")
    ...
    def process_test_images(self, output_dir: Optional[Union[str, Path]] = None) -> List[Dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._process_single_test_image, img_path, known_embeddings, output_dir): img_path
        ...
        self.processing_time = time.time() - start_time
        print(f"Processed {self.processed_images} test images in {self.processing_time:.2f} seconds")
    ...
    def _prepare_known_embeddings(self) -> Dict:
        ...
        if not known_embeddings:
            ...
            if original_path.exists():
                ...
                if img is not None:
                    # Detect faces
                    face_bboxes = self.detector.detect_faces(img, force_detection=True, use_cache=True)
