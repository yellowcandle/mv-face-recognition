...
def plot_bar(matches: List[Tuple[str, float]], max_items: int = 10) -> Figure:
    ...
    try:
        ...
        ax.text(
            0.6, ax.get_ylim()[1] * 0.02, "Threshold", horizontalalignment="center", color="red"
        )
        ...
        if len(embeddings_array) > 1:
            ...
            if embeddings_array.shape[0] >= 2:  # Need at least 2 samples for PCA
                ...
                for category, color, marker in [
                    ...
                    if not subset.empty:
                        ax.scatter(
                            ...
                            alpha=0.7,
                            marker=marker,
                            s=100 if category == "detected" else 70,
                        )
                ...
                ax.text(
                    0.5,
                    0.5,
                    "Not enough data for dimensionality reduction",
                    horizontalalignment="center",
                    verticalalignment="center",
                    transform=ax.transAxes,
                    fontsize=14,
...
def draw_faces_with_labels(
    ...
    try:
        ...
        for face in faces:
            ...
            # Add text background
            text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, font_scale, box_thickness)[
                0
            ]
            cv2.rectangle(
                img_with_faces,
