...
class ModelFinder:
    ...
    def _check_project_models(self, model_name: str) -> Optional[str]:
        ...
        if (
            ...
            and self._is_valid_model_size(model_path, model_name)
        ):
        ...
        for rel_path in model_info.get("package_paths", []):
            for pkg_path in self.package_paths:
                ...
                if (
                    ...
                    and self._is_valid_model_size(full_path, model_name)
                ):
        ...
        for pkg_path in self.package_paths:
            for root, _, files in os.walk(pkg_path):
                if model_name in files:
                    ...
                    if os.path.isfile(full_path) and self._is_valid_model_size(
        ...
        for url in model_info.get("urls", []):
            try:
                    ...
                    with urllib.request.urlopen(request, timeout=30) as response:
                        ...
                        while True:
                            ...
                            if file_size > 0:
                                ...
                                sys.stdout.write(
                                    f"\rDownloading: {percent}% ({bytes_downloaded}/{file_size} bytes)"
                                )
                                sys.stdout.flush()
                ...
                if self._is_valid_model_size(target_path, model_name):
                    ...
                    logger.warning("Downloaded file size is invalid")
                    try:
