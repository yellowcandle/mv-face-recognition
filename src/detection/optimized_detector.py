...
class OptimizedFaceDetector:
    ...
    def __init__(
        ...
        self.model_path = os.path.join(
            self.project_root, "models", "face_detection_yunet.onnx"
        )
    ...
    def _init_detector(self):
        ...
        self.detector = cv2.FaceDetectorYN.create(
            self.model_path, "", self.model_size, self.confidence_threshold
        )
    ...
    def detect_faces(self, image, force_detection=False, use_cache=True):
        ...
        if self.frame_count % 100 == 0 and (self.cache_hits + self.cache_misses) > 0:
            ...
            print(
                f"Face detection cache: {hit_rate:.1f}% hit rate ({self.cache_hits} hits, {self.cache_misses} misses)"
            )
    ...
    def _cache_face(self, face_id, face):
        ...
        self.face_cache[face_id] = {"face": face, "timestamp": time.time()}
