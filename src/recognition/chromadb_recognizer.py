...
class ChromaDBFaceRecognizer:
    ...
    def _setup_chromadb(self):
        ...
        try:
            ...
            try:
                ...
                print(f"Loaded existing ChromaDB collection with {self.collection.count()} embeddings")
            except Exception:
    ...
    @profile_execution
    def add_embedding(self, face_id: str, embedding: np.ndarray, metadata: Optional[Dict[str, Any]] = None):
        """
    ...
    def identify_faces(self, 
                       image: np.ndarray, 
                       face_detector, 
                       face_processor=None) -> List[Dict[str, Any]]:
        """
        ...
        for _, contestant in contestant_info.iterrows():
            ...
            if embedding_path.exists():
                try:
                    ...
                    if isinstance(embedding, list) or (isinstance(embedding, np.ndarray) and embedding.ndim > 1):
                        # Multiple embeddings
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            "total_embeddings": self.collection.count() if self.collection else len(getattr(self, '_fallback_embeddings', {})),
