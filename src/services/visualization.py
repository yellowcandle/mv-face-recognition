...
class VisualizationService:
    ...
    def draw_face_annotations(
        self,
        frame: np.ndarray,
        matches: List[Tuple[Any, str, float]], # (bbox or face_obj, name, confidence)
        timestamp: Optional[str] = None,
        box_thickness: int = 3,
        label_font_size: int = 60,
        timestamp_font_size: int = 40,
    ) -> np.ndarray:
        """
        Draws bounding boxes, labels, and a timestamp on the given frame.

        Args:
            frame (np.ndarray): The image frame to draw on (BGR format).
            matches (List[Tuple[Any, str, float]]): A list of tuples, each containing
                                                     (face_object or bbox, name, confidence).
            timestamp (str): The timestamp to display on the frame.
            box_thickness: Thickness of the bounding box.
            label_font_size: Font size for face labels.
            timestamp_font_size: Font size for the timestamp.

        Returns:
            np.ndarray: The frame with drawn boxes, labels, and timestamp (RGB format).
        """
        try:
            ...
            # Use pre-loaded fonts, or load dynamically if sizes change
            label_font = self.label_font
            timestamp_font = self.timestamp_font
            if label_font_size != 60 or timestamp_font_size != 40: # Reload if sizes are different
                font_path = os.path.join(self.fonts_dir, "SourceHanSansTC-VF.ttf")
                try:
                    label_font = ImageFont.truetype(font_path, label_font_size)
                    timestamp_font = ImageFont.truetype(font_path, timestamp_font_size)
                except Exception as e:
                    logger.warning(f"Could not load custom font with specified size: {e}. Using default.")
                    label_font = ImageFont.load_default()
                    timestamp_font = ImageFont.load_default()

            for item, name, confidence in matches:
                # Determine bbox: if it's an InsightFace Face object, use face.bbox, else assume it's a list/tuple bbox
                bbox = item.bbox.astype(int) if hasattr(item, 'bbox') else np.array(item).astype(int)
                ...
                draw.rectangle(bbox_enlarged, outline="green", width=box_thickness)
                ...
                text_y = max(0, bbox_enlarged[1] - text_height - 5) # Position above bbox
            ...
            return cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) # Return original frame converted to RGB on error
