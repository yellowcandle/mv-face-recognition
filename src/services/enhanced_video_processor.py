...
class FaceTracker:
    ...
    def update_face(
        ...
        self.face_tracks[contestant_name].append({"bbox": bbox, "frame": frame_number})
...
class EnhancedVideoProcessor:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.processed_videos_dir = Path(
            self.config["paths"].get("processed_videos_dir", "processed_videos")
        )
        ...
        self.similarity_threshold = self.config["face_matching"].get(
        ...
        if self.ffmpeg_available:
            logger.info(
                "FFmpeg available - audio tracks will be preserved in annotated videos"
            )
        else:
            logger.warning(
                "FFmpeg not available - annotated videos will not have audio tracks"
            )
        ...
        with tqdm(total=len(videos), desc="Processing videos", unit="video") as pbar:
            for i, video_name in enumerate(videos):
                ...
                try:
                    ...
                    metadata_file = (
                        self.metadata_dir / f"{Path(video_name).stem}_metadata.json"
                    )
                    annotated_video_file = (
                        self.processed_videos_dir
                        / f"{Path(video_name).stem}_annotated.mp4"
                    )
                    ...
                    if (
                        ...
                        logger.info(
                            f"Video {video_name} already processed, skipping..."
                        )
                        ...
                        def video_progress_callback(
                            step, total_steps, current_step_desc
                        ):
                        ...
                        result = self.process_video_comprehensive(
                            video_name, video_progress_callback
                        )
    ...
    def process_video_comprehensive(
        self, video_name: str, progress_callback: Optional[Callable] = None
    ) -> Dict:
        """
        ...
        result = {
            ...
            "stats": {
                ...
                "unique_contestants": len(
                    recognition_results["contestant_appearances"]
                ),
        ...
        logger.info(
            f"Comprehensive processing of {video_name} completed in {processing_time:.2f}s"
        )
        return result

    def create_enhanced_annotated_video(
        self, video_name: str, recognition_results: Dict
    ) -> str:
        """
        ...
        if self.ffmpeg_available:
            ...
            return self._create_annotated_video_with_ffmpeg(
                input_path, output_path, recognition_results
            )
        else:
            # Fallback to OpenCV-only method (no audio)
            return self._create_annotated_video_opencv_only(
                input_path, output_path, recognition_results
            )

    def _create_annotated_video_with_ffmpeg(
        self, input_path: Path, output_path: Path, recognition_results: Dict
    ) -> str:
        """
        ...
        try:
            ...
            ffmpeg_cmd = [
                ...
                "-i",
                str(temp_video_path),  # Annotated video (no audio)
                "-i",
                str(input_path),  # Original video (with audio)
                "-c:v",
                "copy",  # Copy video stream from annotated video
                "-c:a",
                "copy",  # Copy audio stream from original video
                "-map",
                "0:v:0",  # Map video from first input (annotated)
                "-map",
                "1:a:0?",  # Map audio from second input (original), optional
            ...
            if result.returncode == 0:
                logger.info(
                    f"Successfully created annotated video with audio: {output_path}"
                )
        ...
        try:
            while True:
                ...
                if frame_num in frame_lookup:
                    ...
                    frame_result = self._apply_temporal_smoothing(
        ...
        for face in frame_result["faces"]:
            ...
            if face.get("matched", False) and face.get("contestant_name"):
                ...
                if original_bbox != smoothed_bbox:
                    logger.debug(
                        f"Frame {frame_number}: Smoothed {contestant_name} bbox from {original_bbox} to {smoothed_bbox}"
                    )
    ...
    def _draw_enhanced_annotations(
        self, frame: np.ndarray, frame_result: Dict, contestant_colors: Dict
    ) -> np.ndarray:
    ...
    def generate_video_metadata(
        ...
        for name, appearances in recognition_results["contestant_appearances"].items():
            ...
            contestant_timeline[name] = {
                ...
                "first_appearance_time": appearances["first_appearance"]
                / video_info.get("fps", 30),
                "last_appearance_time": appearances["last_appearance"]
                / video_info.get("fps", 30),
    ...
    def extract_highlight_clips(
        self, video_name: str, recognition_results: Dict, clip_duration: int = 10
    ) -> List[Dict]:
        ...
        for contestant_name, appearances in recognition_results[
            ...
            for frame_result in recognition_results["frame_results"]:
                for face in frame_result["faces"]:
                    if (
                        face.get("contestant_name") == contestant_name
                        and face.get("recognition_confidence", 0) > 0.7
                    ):
            ...
            segments = self._find_continuous_segments(
                high_confidence_frames, min_gap=30
            )
    ...
    def _find_continuous_segments(
        self, frame_numbers: List[int], min_gap: int = 30
    ) -> List[List[int]]:
    ...
    def _extract_clip(
        self, video_name: str, start_frame: int, end_frame: int, clip_name: str
    ) -> Optional[Dict]:
