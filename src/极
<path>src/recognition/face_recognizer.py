...
class FaceRecognizer:
    def __init__(
        ...
        if self.use_arcface:
            ...
            if not os.path.exists(recognition_model_path) or极
    ...
    def add_face(self, image: np.ndarray, person_id: str):
        ...
        face_embedding = self._get_embedding极
    ...
    def _get_embedding(self, face_image: np.ndarray) ->极
    def _get_embedding(self, face_image: np.ndarray) -> np.ndarray:
        try:
            ...
            face_image = face_image.astype(np.float极
            face_image = face_image.astype(np.float32) / 255.0

            if self.use_arcface:
                face_image = (face_image - self.mean) / self.std
                face_image = np.transpose(face_image, (2, 极
    ...
    def detect_and_embed(self, image, bboxes=None):
        ...
        for bbox in bboxes:
            try:
                ...
                elif isinstance(bbox, list) and len极
