...
class FaceTrajectory:
    ...
    def update_trajectory(self, bbox: np.ndarray, embedding: Optional[np.ndarray] = None, 
        ...
        if len(self.bbox_history) == 1:
            ...
            self.kalman_filter.statePre = np.array([center_x, center_y, 0, 0], dtype=np.float32)
            self.kalman_filter.statePost = np.array([center_x, center_y, 0, 0], dtype=np.float32)
        else:
            # Predict and update
        ...
        if len(self.bbox_history) >= 3:
            ...
            if len(centers) >= 3:
                ...
                for i in range(1, len(centers)):
                    vel = np.linalg.norm(np.array(centers[i]) - np.array(centers[i-1]))
                ...
                if len(velocities) >= 2:
                    ...
                    self.motion_consistency = max(0.0, 1.0 - (vel_std / max(vel_mean, 1.0)))
    ...
    def get_overall_confidence(self) -> float:
        ...
        detection_conf = np.mean(list(self.confidence_history)) if self.confidence_history else 0.0
...
class FaceTracker:
    ...
    def _calculate_combined_similarity(self, bbox1: np.ndarray, bbox2: np.ndarray,
                                     emb1: Optional[np.ndarray], emb2: Optional[np.ndarray]) -> float:
    ...
    def update(self, detections: List[Tuple[Any, str]], frame_number: int) -> Dict[str, FaceTrajectory]:
        """
        ...
        for detection in detection_data:
            ...
            for track_id, trajectory in self.trajectories.items():
                ...
                ref_bbox = trajectory.predicted_bbox if trajectory.predicted_bbox is not None else trajectory.bbox_history[-1]
                ref_embedding = trajectory.embedding_history[-1] if trajectory.embedding_history else None
                
                similarity = self._calculate_combined_similarity(
                    detection['bbox'], ref_bbox,
                    detection['embedding'], ref_embedding
                )
    ...
    def get_stable_trajectories(self) -> Dict[str, FaceTrajectory]:
        ...
        for track_id, trajectory in self.trajectories.items():
            ...
            if (len(trajectory.bbox_history) >= self.min_trajectory_length or
                trajectory.get_overall_confidence() > 0.6):
    ...
    def get_trajectory_stats(self) -> Dict[str, Any]:
        ...
        if self.trajectories:
            confidences = [t.get_overall_confidence() for t in self.trajectories.values()]
