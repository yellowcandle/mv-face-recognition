...
class VideoProcessor:
    ...
    def process_video(
        ...
        try:
            ...
            while True:
                ...
                self.stats["faces_recognized"] += sum(
                    1 for r in frame_results if r.get("person_id") or r.get("name")
                )
    ...
    def _process_frame(
        self, frame: np.ndarray, timestamp: float, video_name: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _annotate_frame(
        self, frame: np.ndarray, results: List[Dict[str, Any]], timestamp: float
    ) -> np.ndarray:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["videos_processed"] > 0:
            stats["avg_video_processing_time"] = (
                stats["processing_time"] / stats["videos_processed"]
            )
        else:
