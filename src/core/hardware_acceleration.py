...
class HardwareAccelerator:
    ...
    def __init__(self):
        ...
        logger.info(
            f"System: {self.system_info['platform']} {self.system_info['machine']}"
        )
        ...
        try:
            ...
            if "CUDAExecutionProvider" in all_providers:
                ...
                if self._is_cuda_available() or self._is_cloud_environment():
                    ...
                    logger.warning(
                        "⚠️ CUDA provider available but CUDA runtime not detected"
                    )
            else:
    ...
    def _is_cloud_environment(self) -> bool:
        ...
        if any(cloud_indicators):
            logger.debug(
                "Cloud environment detected, trusting ONNXRuntime CUDA detection"
            )
            return True
    ...
    def print_hardware_info(self):
        ...
        print(
            f"  Cloud Environment: {'✅ Yes' if self._is_cloud_environment() else '❌ No'}"
        )
