...
class StandardFaceRecognizer(FaceRecognizer):
    ...
    def __init__(
        ...
        self.embedding_cache = EmbeddingCache(
            cache_dir=self.cache_dir, memory_size=embedding_cache_size
        )
        ...
        try:
            ...
            try:
                ...
                model_finder = ModelFinder(
                    project_root=str(self.project_root), cache_dir=str(self.cache_dir)
                )
            ...
            providers = []
            if (
                self.face_detector.device == "cuda"
            ...
            print(
                f"Model input shape: {self.input_shape}, using size: {self.input_width}x{self.input_height}"
            )
            ...
            try:
                ...
                subprocess.run(
                    [
                        "python",
                        str(self.project_root / "download_models.py"),
                        "--force",
                    ],
                    check=True,
    ...
    def batch_identify_faces(
        self, images: List[np.ndarray], known_embeddings: Dict[str, List[np.ndarray]]
