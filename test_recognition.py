        ...
        try:
            ...
            if len(arr.shape) == 1:
                ...
                if arr.shape[0] != embedding_dimensions:
                    print(f"Embedding in {os.path.basename(f)} has dimension {arr.shape[0]}, "
                ...
                print(f"Skipping embedding file {os.path.abspath(f)}: not a 1D array, shape={arr.shape}")
...
def get_top_matches(face_embedding, gallery_embeddings, gallery_nicknames, top_n=5, threshold=0.4):
    ...
    if norm_face_embedding.shape[0] != gallery_embeddings.shape[1]:
        print(f"Face embedding dimension {norm_face_embedding.shape[0]} doesn't match gallery "
...
def draw_faces_with_labels(image, faces, matches_list):
    ...
    for i, (face, matches) in enumerate(zip(faces, matches_list)):
        ...
        cv2.putText(img_copy, f"{i+1}: {label}", (x1, y1 - 10), 
...
def main():
    ...
    for threshold in thresholds:
        ...
        face_recognizer = FaceRecognizer(face_detector=core_detector, 
        ...
        for i, face in enumerate(faces):
            ...
            if face_img is not None:
                ...
                top_matches = get_top_matches(embedding, gallery_embeddings, gallery_nicknames, 
                                              top_n=3, threshold=threshold)
