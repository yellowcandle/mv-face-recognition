...
def draw_arrow(img, start_point, end_point, color, thickness=3, arrow_size=20):
    ...
    new_start = (int(start_point[0] + dx * margin),
                int(start_point[1] + dy * margin))
    new_end = (int(end_point[0] - dx * margin),
              int(end_point[1] - dy * margin))
    ...
    p1 = (int(new_end[0] - arrow_size * np.cos(angle - arrow_angle)),
          int(new_end[1] - arrow_size * np.sin(angle - arrow_angle)))
    p2 = (int(new_end[0] - arrow_size * np.cos(angle + arrow_angle)),
          int(new_end[1] - arrow_size * np.sin(angle + arrow_angle)))
...
def draw_utf8_text(img, text, pos, font_size, color):
    ...
    try:
        ...
        bg_img = Image.new('RGBA', (bg_right - bg_left, bg_bottom - bg_top), (0, 0, 0, 0))
        bg_draw = ImageDraw.Draw(bg_img)
        bg_draw.rectangle([(0, 0), (bg_right - bg_left, bg_bottom - bg_top)], 
        ...
        img_copy[bg_top:bg_bottom, bg_left:bg_right] = result_img[bg_top:bg_bottom, bg_left:bg_right]
...
def get_label_dimensions(text, font_size):
    ...
    bbox = draw.textbbox((0, 0), text, font=font)
    return (bbox[2] - bbox[0] + LABEL_PADDING * 2, 
            bbox[3] - bbox[1] + LABEL_PADDING * 2)

def find_label_position(img, label_width, label_height, face_bbox, existing_labels, seg_mask):
    ...
    search_areas = [
        ...
        {
            'x_range': range(min(img_width - label_width, face_bbox[2]), min(img_width - label_width, face_bbox[2] + 400), 10),
            'y_range': range(max(0, face_center_y - 100), min(img_height - label_height, face_center_y + 100), 10),
            'base_cost': 0  # Preferred position
        },
        # Left side (wider range)
        {
            'x_range': range(max(0, face_bbox[0] - 400), max(0, face_bbox[0] - label_width), 10),
            'y_range': range(max(0, face_center_y - 100), min(img_height - label_height, face_center_y + 100), 10),
            'base_cost': 50  # Slightly less preferred
        },
        # Top (last resort)
        {
            'x_range': range(max(0, face_center_x - 200), min(img_width - label_width, face_center_x + 200), 10),
            'y_range': range(max(0, face_bbox[1] - 200), max(0, face_bbox[1] - label_height), 10),
    ...
    for area in search_areas:
        for test_x in area['x_range']:
            for test_y in area['y_range']:
                ...
                label_region = (slice(test_y, test_y + label_height),
                              slice(test_x, test_x + label_width))
                
                # Cost from segmentation mask (avoid placing on faces/people)
                if label_region[0].stop <= seg_mask.shape[0] and label_region[1].stop <= seg_mask.shape[1]:
                ...
                for ex_x, ex_y, ex_w, ex_h in existing_labels:
                    if (test_x < ex_x + ex_w and test_x + label_width > ex_x and
                        test_y < ex_y + ex_h and test_y + label_height > ex_y):
                        overlap_area = (min(test_x + label_width, ex_x + ex_w) - max(test_x, ex_x)) * \
    ...
    return best_pos if best_pos is not None else (face_bbox[0], max(0, face_bbox[1] - label_height))
...
for face_idx, face in enumerate(faces):
    ...
    for contestant_name, contestant_embedding in contestants_embeddings.items():
        contestant_embedding_norm = contestant_embedding / np.linalg.norm(contestant_embedding)
    ...
    confidence_threshold = MASKED_CONFIDENCE_THRESHOLD if masked else NORMAL_CONFIDENCE_THRESHOLD
    
    # Print all top 3 matches with their scores
    print(f"\nFace #{face_idx + 1}:")
    print(f"Confidence threshold: {confidence_threshold} {'(masked)' if masked else ''}")
    ...
    for i, (match_name, score) in enumerate(top_3):
        if score >= confidence_threshold:
            ...
            face_center = ((bbox_enlarged[0] + bbox_enlarged[2]) // 2,
                         (bbox_enlarged[1] + bbox_enlarged[3]) // 2)
            
            # Find position for label using segmentation mask
            x_pos, y_pos = find_label_position(test_image, label_width, label_height,
                                             bbox_enlarged, existing_labels, seg_mask)
            ...
            # Draw label
            test_image = draw_utf8_text(test_image, label,
                                      (x_pos, y_pos),
