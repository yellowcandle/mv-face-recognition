...
def load_gallery_embeddings():
    ...
    if not gallery_embeddings:
        ...
        return np.empty((0, 128), dtype=np.float32), [], [] # MODIFIED: Default to 128 for consistency
...
def plot_embedding_scatter(detected_face_embeddings_list: List[np.ndarray], 
    ...
    if num_detected > 0:
        ...
        gallery_dim = gallery_embeddings_global.shape[1] if num_gallery > 0 else (detected_face_embeddings_list[0].shape[0] if detected_face_embeddings_list else 128)
    ...
    if not all_embeddings_list:
        fig, ax = plt.subplots(figsize=(6, 4)); ax.text(0.5, 0.5, "No embeddings for UMAP", ha="center", va="center"); return fig
    all_embeddings_np = np.array(all_embeddings_list)
    if all_embeddings_np.shape[0] < 2:
        fig, ax = plt.subplots(figsize=(6, 4)); ax.text(0.5, 0.5, "Need at least 2 points for UMAP", ha="center", va="center"); return fig
        
    n_neighbors_val = min(15, all_embeddings_np.shape[0] - 1)
    if n_neighbors_val < 2: # UMAP requires n_neighbors >= 2
        fig, ax = plt.subplots(figsize=(6, 4)); ax.text(0.5, 0.5, f"Too few points ({all_embeddings_np.shape[0]}) for UMAP.", ha="center", va="center"); return fig
    ...
    try:
        ...
        fig, ax = plt.subplots(figsize=(6, 4)); ax.text(0.5, 0.5, f"UMAP error: {e}", ha="center", va="center", fontsize=8); return fig

    fig, ax = plt.subplots(figsize=(12, 10))
    color_map = {'gallery': 'blue', 'detected_recognized': 'red', 'detected_unknown': 'orange', 'nearest_gallery': 'green'}
    size_map = {'gallery': 30, 'detected_recognized': 100, 'detected_unknown': 70, 'nearest_gallery': 50}
    ...
    if num_gallery > 0:
        ...
        for i in range(num_gallery):
            if not any(i in nearest_list for nearest_list in nearest_gallery_indices_for_detected):
                 ax.scatter(gallery_2d[i, 0], gallery_2d[i, 1], c=color_map['gallery'], s=size_map['gallery'], alpha=0.5, label="Gallery (Other)" if 'Gallery (Other)' not in plt.gca().get_legend_handles_labels()[1] else "")

    if num_detected > 0:
        detected_2d = embedding_2d[num_gallery:]
        for i in range(num_detected):
            detected_point = detected_2d[i]; ptype = point_types[num_gallery + i]; plabel = point_labels[num_gallery + i]
            ax.scatter(detected_point[0], detected_point[1], c=color_map[ptype], s=size_map[ptype], label=plabel.split(":")[0] if plabel.split(":")[0] not in plt.gca().get_legend_handles_labels()[1] else "", alpha=0.9, edgecolors='black' if ptype == 'detected_recognized' else None, marker='o' if ptype == 'detected_recognized' else 'X')
            ...
            for gallery_idx in nearest_gallery_indices_for_detected[i]:
                if 0 <= gallery_idx < num_gallery:
                    ...
                    ax.scatter(gallery_match_point[0], gallery_match_point[1], c=color_map['nearest_gallery'], s=size_map['nearest_gallery'], alpha=0.8, edgecolors='black', marker='s', label="Nearest Gallery Match" if "Nearest Gallery Match" not in plt.gca().get_legend_handles_labels()[1] else "")
                    ax.plot([detected_point[0], gallery_match_point[0]], [detected_point[1], gallery_match_point[1]], c='gray', linestyle='--', linewidth=0.8, alpha=0.7)

    ax.set_title("Embedding Space (UMAP) with Nearest Matches"); ax.set_xlabel("UMAP Dimension 1"); ax.set_ylabel("UMAP Dimension 2")
    handles, labels = plt.gca().get_legend_handles_labels(); by_label = dict(zip(labels, handles)); ax.legend(by_label.values(), by_label.keys(), loc='best')
    plt.tight_layout(); return fig

def overlay_faces(frame: np.ndarray, faces: List[InsightFaceObject], matches: List[List[Tuple[str, float]]]) -> np.ndarray:
    frame_pil = Image.fromarray(frame); draw = ImageDraw.Draw(frame_pil)
    try: font = ImageFont.truetype(CJKV_FONT_PATH, FONT_SIZE)
    except Exception as e: logger.warning(f"Could not load font: {e}"); font = ImageFont.load_default()
    ...
    for face, match_list in zip(faces, matches):
        ...
        frame_pil = Image.fromarray(current_frame_np); draw = ImageDraw.Draw(frame_pil) # Re-initialize draw object
...
def process_frame(frame: np.ndarray,
    ...
    empty_fig, ax = plt.subplots(figsize=(4,3) if vis_type == "bar" else (6,2)); ax.set_xticks([]); ax.set_yticks([])
    if not raw_detected_faces:
        logger.debug("No faces detected"); ax.text(0.5,0.5, "No faces detected", ha="center", va="center"); plt.tight_layout(); return frame, empty_fig

    detected_faces = [f for f in raw_detected_faces if not hasattr(f, "det_score") or f.det_score >= DETECTION_SCORE_THRESHOLD]
    logger.debug(f"Number of faces after filtering: {len(detected_faces)}")
    if not detected_faces:
        ax.text(0.5,0.5, f"No faces above score threshold ({DETECTION_SCORE_THRESHOLD})", ha="center", va="center", fontsize=8); plt.tight_layout(); return frame, empty_fig
    plt.close(empty_fig) # Close if not used

    matches = []; all_sims = []; detected_embeds = []
    for face in detected_faces:
        face_img = core_detector.extract_face(frame, face.bbox.astype(int), padding=0.1)
        current_embedding = None; sims_for_this_face = np.array([])
        if face_img is not None and face_img.size > 0:
            current_embedding = face_recognizer_instance._get_embedding(face_img) # MODIFIED: Use FaceRecognizer
            if current_embedding is not None and current_embedding.size > 0:
                detected_embeds.append(current_embedding)
                top_matches_with_indices, sims_for_this_face = get_top_matches(current_embedding, gallery_embeddings, gallery_display_names)
                matches.append(top_matches_with_indices)
            else: logger.debug("Failed to get embedding"); matches.append([])
        else: logger.debug("Failed to extract face image"); matches.append([])
    ...
    if not any(matches):
        ...
        fig = plot_bar([]) if vis_type == "bar" else plot_embedding_scatter(detected_embeds, gallery_embeddings, gallery_display_names, matches)
        return processed_frame, fig

    if vis_type == "bar":
        all_top_matches_for_plot = sorted([(f"Face {i+1}: {name}", score) for i, mlist in enumerate(matches) if mlist for name, score, _ in mlist], key=lambda x: x[1], reverse=True)
...
def get_total_frames(video_path: str) -> int:
    cap = cv2.VideoCapture(video_path); total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)); cap.release(); return total

# MODIFIED: Added face_recognizer_instance parameter
def get_frame_by_index(video_path: str, frame_idx: int, core_detector: FaceDetector, face_recognizer_instance: FaceRecognizer, gallery_embeddings: np.ndarray, gallery_display_names: List[str], vis_type: str = "bar") -> Optional[Tuple[np.ndarray, plt.Figure]]:
    cap = cv2.VideoCapture(video_path); cap.set(cv2.CAP_PROP_POS_FRAMES, frame_idx); ret, frame = cap.read(); cap.release()
    if not ret: logger.warning(f"Failed to read frame {frame_idx} from {video_path}"); return None
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    # MODIFIED: Pass face_recognizer_instance
    return process_frame(frame_rgb, core_detector, face_recognizer_instance, gallery_embeddings, gallery_display_names, vis_type)
...
def build_gradio_interface():
    ...
    logger.info(f"Initialized FaceRecognizer for Gradio with embedding size: {face_recognizer_instance.embedding_size}")
    if face_recognizer_instance.embedding_size != 128:
        logger.warning(f"Gradio's FaceRecognizer is NOT using 128-dim embeddings. Current: {face_recognizer_instance.embedding_size}. Check SFace model.")
    ...
    def update_slider_on_video(selected_video):
        video_path = video_paths[selected_video]; total = get_total_frames(video_path)
    ...
    def update_frame(selected_video, frame_idx, vis_type):
        ...
        # MODIFIED: Pass face_recognizer_instance
        result = get_frame_by_index(video_path, frame_idx, core_detector, face_recognizer_instance, gallery_embeddings, gallery_display_names, vis_type)
        if result is None: return None, None
        out_frame, fig = result; return out_frame, fig
    ...
    def play_loop(selected_video, vis_type, state_in): # Renamed state to state_in for clarity
        ...
        if not state_in['playing']:
            if current_frame_idx < total_frames: # If not at the end
                ...
                result = get_frame_by_index(video_path, current_frame_idx, core_detector, face_recognizer_instance, gallery_embeddings, gallery_display_names, vis_type)
                if result:
                    ...
                    yield (out_frame, fig, current_frame_idx, {"playing": True, "frame": current_frame_idx + 1, "total": total_frames})
                    return # End this iteration, Gradio calls again with new state if playing is True
                else: # Failed to get the first frame
                    yield (gr.update(), gr.update(), current_frame_idx, {"playing": False, "frame": current_frame_idx, "total": total_frames})
                    return
    ...
    with gr.Blocks(title="Face Recognition System", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3): video_dropdown = gr.Dropdown(choices=video_files, label="Select Video File", value=video_files[0] if video_files else None, interactive=True)
            with gr.Column(scale=2): vis_type = gr.Radio(["bar", "scatter"], value="bar", label="Visualization Type", interactive=True)
        with gr.Row():
            with gr.Column(scale=4): frame_slider = gr.Slider(minimum=0, maximum=1, value=0, step=1, label="Frame Timeline", interactive=True)
            with gr.Column(scale=1):
                with gr.Row(): play_btn = gr.Button("▶️ Play", variant="primary"); pause_btn = gr.Button("⏸️ Pause", variant="secondary")
        with gr.Row():
            with gr.Column(scale=3): output_video = gr.Image(label="Video Frame with Recognized Faces")
            with gr.Column(scale=2): output_plot = gr.Plot(label="Similarity Visualization")
        gr.Markdown("### Instructions")
        with gr.Accordion("Help", open=False): gr.Markdown("- **Select Video**: Choose a video file from the dropdown\n- **Visualization Type**: \n    - **Bar**: Shows similarity scores for recognized faces\n    - **Scatter**: Shows UMAP projection of face embeddings\n- **Frame Timeline**: Drag to navigate through the video\n- **Play/Pause**: Control video playback")
        state = gr.State({"playing": False, "frame": 0, "total": 1})
        video_dropdown.change(fn=update_slider_on_video, inputs=[video_dropdown], outputs=[frame_slider, state])
        frame_slider.change(fn=update_frame, inputs=[video_dropdown, frame_slider, vis_type], outputs=[output_video, output_plot])
        vis_type.change(fn=update_frame, inputs=[video_dropdown, frame_slider, vis_type], outputs=[output_video, output_plot])
        play_btn.click(fn=play_loop, inputs=[video_dropdown, vis_type, state], outputs=[output_video, output_plot, frame_slider, state], api_name=False)
        pause_btn.click(fn=lambda s: {"playing": False, "frame": s["frame"], "total": s["total"]}, inputs=[state], outputs=[state], api_name=False)
    return demo
