...
def benchmark_matching(video_path, std_embeddings, chroma_embeddings, std_recognizer, detector, iterations=3, frame_limit=100, verbose=False):
...
def plot_results(benchmark_results, output_dir=None):
    ...
    std_times = np.array([item for sublist in benchmark_results["standard"]["times"] for item in sublist]) * 1000
    chroma_times = np.array([item for sublist in benchmark_results["chromadb"]["times"] for item in sublist]) * 1000
