...
def test_audio_preservation():
    ...
    logger.info(
        f"Found {len(recognition_results.get('frame_data', []))} processed frames"
    )
    ...
    recognition_results["contestant_appearances"] = recognition_results.get(
        "contestant_timeline", {}
    )

    # Create new annotated video with audio preservation
    output_path = processor.create_enhanced_annotated_video(
        test_video, recognition_results
    )
