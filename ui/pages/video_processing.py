...
def create():
    ...
    with ui.splitter(value=70).classes('w-full h-full') as splitter:
        with splitter.before:
            with ui.column().classes('w-full'):
                with ui.row().classes('w-full items-center'):
                    ...
                    if not video_files:
                        ui.label("No videos found in /source/videos/").classes('text-negative')
                    else:
                        ui.select(
                            options=video_files,
        ...
        with splitter.after:
            with ui.column().classes('p-4 w-full'):
                ...
                ui.button('Start Processing', on_click=start_processing_click, icon='play_arrow').props('color=primary')
    ...
    async def start_processing(video_name, start, end):
        ...
        ui.notify(f"Starting processing for {video_name} from {start}s to {end}s", type='info')
        ...
        try:
            ...
            for frame_num, annotated_frame, face_results, frame_stats in processor:
                ...
                # more detailed stats.
                progress.set_value(frame_stats['total_frames_processed'] / frame_stats['estimated_total_frames'])
                
                if face_results:
                    with results_area:
                        for result in face_results:
                            if result['matched']:
                                ui.chip(f"{result['contestant_name']} ({result['recognition_confidence']:.2f})", icon='face').props('color=positive')
                
                await ui.run_javascript('window.dispatchEvent(new Event("resize"))', respond=False) # Force redraw
